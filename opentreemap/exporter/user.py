...
def _write_users_csv(csv_obj, instance, min_join_ts=None, min_edit_ts=None):
    field_names = [
        ...
        "last_edit_model",
        "last_edit_model_id",
        "last_edit_instance_id",
        "last_edit_field",
        "last_edit_previous_value",
        "last_edit_current_value",
        "last_edit_user_id",
        "last_edit_action",
        "last_edit_requires_auth",
        "last_edit_ref",
        "last_edit_created",
    ]
...
def _users_export(instance, min_join_ts, min_edit_ts):
    ...
    if min_edit_ts:
        with _date_filter(min_edit_ts, "minEditDate") as min_edit_date:
            user_ids = (
                Audit.objects.filter(instance=instance)
                .filter(Q(created__gt=min_edit_date) | Q(updated__gt=min_edit_date))
                .distinct("user")
...
def _user_as_dict(user, instance):
    ...
    last_edits = Audit.objects.filter(instance=instance, user=user).order_by(
        "-updated"
    )[:1]
    ...
    if last_edits:
        ...
        modeldata.update(
            {"last_edit_%s" % k: v for (k, v) in last_edit.dict().iteritems()}
        )
...
def _date_filter(timestamp, filter_name):
    try:
        ...
        raise ValidationError(
            "%(filter_name)s='%(ts)s' not a valid timestamp "
            "of format: %(format)s"
            % {"ts": timestamp, "format": DATETIME_FORMAT, "filter_name": filter_name}
        )
    yield filter_date
