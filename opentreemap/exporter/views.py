...
from exporter import EXPORTS_NOT_ENABLED_CONTEXT, EXPORTS_FEATURE_DISABLED_CONTEXT
...
def _get_user_extra_args(request):
    return [request.GET.get("minJoinDate"), request.GET.get("minEditDate")]
...
def check_export(request, instance, job_id):
    ...
    if job.user and job.user != request.user:
        return {"status": "ERROR", "message": "Job not initiated by user", "url": None}
