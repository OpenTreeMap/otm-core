...
from api.views import (
    status_view,
    version_view,
    public_instances_endpoint,
    remove_current_tree_from_plot,
    plots_endpoint,
    species_list_endpoint,
    approve_pending_edit,
    reject_pending_edit,
...
urlpatterns = [
    ...
    url(instance_pattern + r"/plots/(?P<plot_id>\d+)/tree/photo$", add_photo_endpoint),
    url(
    ...
    url(instance_pattern + r"/plots/(?P<plot_id>\d+)$", plot_endpoint),
    url(
    ...
    url(
        instance_pattern + r"/users.json", export_users_json_endpoint, name="user_json"
    ),
]
