    ...
    def wrapper(request, *args, **kwargs):
        ...
        if request.api_version < 3:
            instance_info_dict["field_key_groups"] = [
                field_group
                for field_group in instance_info_dict["field_key_groups"]
        ...
        if request.api_version < 4:
            multichoice_fields = {
                field
                for field, info in instance_info_dict["fields"].iteritems()
            ...
            instance_info_dict["search"]["standard"] = [
                field
                for field in instance_info_dict["search"]["standard"]
                if field["search_type"] in {"SPECIES", "RANGE", "BOOL"}
            ]

        if request.api_version < 5:
            instance_info_dict["geoRevHash"] = instance_info_dict["universalRevHash"]
...
def instances_closest_to_point(request, lat, lng):
    ...
    if user and not user.is_anonymous():
        user_instance_ids = (
            InstanceUser.objects.filter(user=user)
    ...
    try:
        distance = float(request.GET.get("distance", settings.NEARBY_INSTANCE_RADIUS))
    except ValueError:
        raise HttpBadRequestException("The distance parameter must be a number")

    instances = Instance.objects.filter(get_viewable_instances_filter())
    ...
    return {
        "nearby": get_annotated_contexts(
            instances.filter(is_public=True)
            .filter(bounds__geom__distance_lte=(point, D(m=distance)))
            .exclude(personal_predicate)[0:max_instances]
        ),
        "personal": get_annotated_contexts(instances.filter(personal_predicate)),
    }
...
def instance_info(request, instance):
    ...
    for field_group in mobile_api_fields:
        ...
        if key:
            field_group[key] = [field for field in field_group[key] if field in perms]

    readable_mobile_api_fields = [
        group
        for group in mobile_api_fields
    ...
    info["meta_perms"] = {
        ...
        "can_edit_tree": (
            perms_lib.plot_is_writable(role) or perms_lib.tree_is_writable(role)
        ),
    ...
    info["config"] = {
        x: instance.config[x] for x in instance.config if x in public_config_keys
    }
...
def public_instances(request):
    return _contextify_instances(
        Instance.objects.filter(is_public=True).filter(get_viewable_instances_filter())
    )
