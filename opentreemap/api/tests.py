...
from api.instance import instances_closest_to_point, instance_info, public_instances
...
def assert_reputation(test_case, expected_reputation):
    ...
    test_case.assertEqual(
        expected_reputation,
        reputation,
        "Reputation is %s but %s was expected" % (reputation, expected_reputation),
    )
...
class PlotListing(OTMTestCase):
    ...
    def test_edits(self):
        ...
        get_signed(self.cliend, "%s/user/%s/edits" % (API_PFX, user.pk))
    ...
    def test_basic_data(self):
        ...
        info = self.client.get(
            "%s/instance/%s/plots" % (API_PFX, self.instance.url_name)
        )
...
class Locations(OTMTestCase):
    ...
    def test_locations_plots_endpoint_with_auth(self):
        response = get_signed(
            self.client,
            "%s/instance/%s/locations/0,0/plots" % (API_PFX, self.instance.url_name),
            user=self.user,
        )
    ...
    def test_plots(self):
        ...
        response = get_signed(
            ...
            % (API_PFX, self.instance.url_name, plot.geom.x, plot.geom.y),
        )
...
class CreatePlotAndTree(OTMTestCase):
    ...
    def test_create_plot_with_tree(self):
        ...
        self.assertEqual(200, response.status_code, "Create failed:" + response.content)
    ...
    def test_create_plot_with_invalid_tree_returns_400(self):
        ...
        self.assertTrue(
            "fieldErrors" in body_dict,
            "Expected the body JSON to have a 'fieldErrors' key",
        )

        fieldErrors = body_dict["fieldErrors"]
        self.assertTrue(
            "tree.height" in fieldErrors, 'Expected "treeHeight" to be in "fieldErrors"'
        )

        self.assertEqual("The height is too large", fieldErrors["tree.height"][0])
...
class UpdatePlotAndTree(OTMTestCase):
    ...
    def _makeUdfd(self, name, data_type, model_type="Plot", choices=None):
        set_write_permissions(self.instance, self.user, model_type, ["udf:" + name])
    ...
    def test_invalid_plot_id_returns_404_and_a_json_error(self):
        response = put_json(
            "%s/instance/%s/plots/0" % (API_PFX, self.instance.url_name),
            {},
            self.client,
            self.user,
        )
    ...
    def test_update_plot(self):
        ...
        response = put_json(
            "%s/instance/%s/plots/%d" % (API_PFX, self.instance.url_name, test_plot.pk),
            updated_values,
            self.client,
            self.user,
        )
    ...
    def test_update_tree_with_udfs(self):
        self._makeUdfd("abc", "choice", model_type="Tree", choices=["a", "b", "c"])
        self._makeUdfd("def", "choice", model_type="Tree", choices=["d", "e", "f"])
        self._makeUdfd(
            "multi-ghi", "multichoice", model_type="Tree", choices=["g", "h", "i"]
        )
        self._makeUdfd(
            "multi-jkl", "multichoice", model_type="Tree", choices=["j", "k", "l"]
        )
    ...
    def test_update_plot_with_pending(self):
        ...
        self.assertEqual(
            3,
            len(response_json["pending_edits"].keys()),
    ...
    def test_invalid_field_returns_200_field_is_not_in_response(self):
        ...
        self.assertFalse(
            "foo" in response_json.keys(), "Did not expect foo to be added to the plot"
        )
    ...
    def test_update_tree(self):
        ...
        response = put_json(
            "%s/instance/%s/plots/%d" % (API_PFX, self.instance.url_name, test_plot.id),
            updated_values,
            self.client,
            self.user,
        )
    ...
    def test_update_tree_with_pending(self):
        ...
        self.assertEqual(
            2.3,
            tree.diameter,
            "A pend should have been created instead" " of editing the tree value.",
        )
        self.assertEqual(
            1,
            len(Audit.pending_audits()),
        ...
        self.assertEqual(
            1,
            len(response_json["pending_edits"].keys()),
            "Expected the json response to have a" " pending_edits dict with 1 keys",
        )
    ...
    def assert_pending_edit_operation(self, action, original_dbh=2.3, edited_dbh=3.9):
        ...
        self.assertEqual(
            None, pending_edit.ref, "Expected that the audit has not been applied"
        )
        ...
        response = post_json(
            "%s/instance/%s/pending-edits/%d/%s/"
            % (API_PFX, self.instance.url_name, pending_edit.id, action_str),
            None,
            self.client,
            self.user,
        )
        ...
        self.assertIsNotNone(
            pending_edit.ref, "Expected the audit to be marked as processed"
        )

        pending_edit_marked = pending_edit.ref
        self.assertEqual(
            pending_edit_marked.action,
            action,
            "Expected the type of the audit to be '%s'" % action,
        )
        ...
        if action == Audit.Type.PendingApprove:
            ...
            self.assertEqual(
                original_dbh,
                test_tree.diameter,
                "Expected dbh to NOT have been " "updated on the Tree",
            )
        ...
        if action == Audit.Type.PendingApprove:
            self.assertEqual(
                ...
                "Expected dbh to have been updated" " in the JSON response",
            )
        elif action == Audit.Type.PendingReject:
            self.assertEqual(
                original_dbh,
                response_json["tree"]["dbh"],
                "Expected dbh to NOT have been " "updated in the JSON response",
            )
    ...
    def test_approve_plot_pending_with_mutiple_pending_edits(self):
        ...
        self.assertEqual(
            response.status_code, 200, "Non 200 response when updating plot"
        )
        ...
        self.assertEqual(
            3,
            pending_edit_count,
            "Expected three pending edits but got %d" % pending_edit_count,
        )
        ...
        self.assertEqual(
            response.status_code, 200, "Non 200 response when approving the pend"
        )

        self.assertEqual(
            1,
            len(test_plot.get_active_pending_audits()),
    ...
    def test_remove_plot(self):
        ...
        tree_id = tree.pk
        url = "%s/instance/%s/plots/%d" % (API_PFX, self.instance.url_name, plot_id)

        response = self.client.delete(url, **self.user)
        self.assertEqual(
            403, response.status_code, "Expected 403 when there's still a tree"
        )

        tree.delete_with_user(self.user)
        response = self.client.delete(url, **self.user)
        self.assertEqual(
            200, response.status_code, "Expected 200 status code after delete"
        )

        response_dict = loads(response.content)
        self.assertTrue(
            "ok" in response_dict, 'Expected a json object response with a "ok" key'
        )

        self.assertTrue(
            response_dict["ok"],
            'Expected a json object response with a "ok" key' "set to True",
        )
    ...
    def test_update_collection_udf(self):
        ...
        updated_values = {
            "tree": {"udf:Stewardship": [{"Action": "Watered", "Date": "2014-01-01"}]},
            "plot": {"udf:Stewardship": [{"Action": "Enlarged", "Date": "2014-02-02"}]},
        }
        ...
        self.assertEqual(new_tree.pk, response_json["tree"]["id"])
        self.assertEqual(
    ...
    def test_update_multichoice_v3(self):
        ...
        updated_values = {
            "plot": {"geom": {"y": 0, "x": 0, "srid": 4326}, "udf:multi": None}
        }

        response = put_json(
            "/api/v3/instance/%s/plots/%d" % (self.instance.url_name, test_plot.pk),
            updated_values,
            self.client,
            self.user,
        )
        ...
        updated_values = {
            "plot": {"geom": {"y": 0, "x": 0, "srid": 4326}, "udf:multi": ["b", "c"]}
        }
        ...
        updated_values = {
            "plot": {"geom": {"y": 0, "x": 0, "srid": 4326}, "udf:multi": []}
        }
...
class Instance(LocalMediaTestCase):
    ...
    test_png_path = os.path.join(
        os.path.dirname(__file__), "test_resources", test_png_name
    )
    ...
    def setUp(self):
        ...
        self.instance.save()
        self.instance.logo.save(
            Instance.test_png_name, File(open(Instance.test_png_path, "r"))
        )
    ...
    def test_returns_config_colors(self):
        ...
        self.assertEqual(self.expected_scss_variables, info["config"]["scss_variables"])

    def test_returns_only_scss_variables(self):
        request = sign_request_as_user(make_request(), self.user)
        info = instance_info(request, self.instance)
        self.assertTrue(
            "scss_variables" in info["config"],
            '"scss_variables" should exist in instance.config',
        )
        self.assertEqual(
            1,
            len(info["config"]),
            "The config dict should only contain a single, " "white listed item",
        )
    ...
    def test_removes_unreadable_api_fields(self):
        ...
        set_invisible_permissions(
            self.instance, self.user, "Tree", ["species", "diameter", "height"]
        )
        info = instance_info(request, self.instance)

        self.assertDictContainsSubset(
            {"field_keys": ["tree.date_planted"]}, info.get("field_key_groups")[0]
        )
    ...
    def test_multichoice_fields_v4(self):
        ...
        self.assertTrue(
            any(
                "plot.udf:multi" in group.get("field_keys", [])
    ...
    def _get_search_ids(self, info_dict):
        return [field["identifier"] for field in info_dict["search"]["standard"]]
...
class InstancesClosestToPoint(OTMTestCase):
    ...
    def test_nearby_list_distance(self):
        request = sign_request_as_user(make_request({"distance": 100000}), self.user)
    ...
    def test_user_list_max(self):
        request = sign_request_as_user(
            make_request({"max": 3, "distance": 100000}, user=self.user), self.user
        )
...
class TreePhotoTest(LocalMediaTestCase):
    test_jpeg_path = os.path.join(
        os.path.dirname(__file__), "test_resources", "2by2.jpeg"
    )

    test_png_path = os.path.join(
        os.path.dirname(__file__), "test_resources", "2by2.png"
    )
    ...
    def _test_post_photo(self, path):
        ...
        url = "%s/instance/%s/plots/%d/tree/photo" % (
            API_PFX,
            self.instance.url_name,
            plot_id,
        )
        ...
        with open(path) as img:
            req = self.factory.post(url, {"name": "afile", "file": img})

            req = sign_request_as_user(req, self.user)

            response = add_photo_endpoint(
                req, LATEST_API, self.instance.url_name, plot_id
            )
...
class UserTest(LocalMediaTestCase):
    ...
    def make_post_request(self, datadict):
        r = sign_request(make_request(method="POST", body=dumps(datadict)))
    ...
    def testUploadPhoto(self):
        ...
        url = reverse("update_user_photo", kwargs={"version": 3, "user_id": peon.pk})
        ...
        with open(TreePhotoTest.test_jpeg_path) as img:
            ...
            response = update_profile_photo_endpoint(req, LATEST_API, str(peon.pk))
    ...
    def testCanOnlyUploadAsSelf(self):
        ...
        with open(TreePhotoTest.test_jpeg_path) as img:
            ...
            response = update_profile_photo_endpoint(req, LATEST_API, str(grunt.pk))
    ...
    def testMissingFields(self):
        ...
        self.assertRaises(
            ValidationError, create_user, self.make_post_request(self.defaultUserDict)
        )
    ...
    def test_boolean_defaults_empty(self):
        user_id = create_user(self.make_post_request(self.defaultBaseUserDict))["id"]
    ...
    def test_boolean_defaults_partial(self):
        user_id = create_user(self.make_post_request(self.defaultUserDict))["id"]
        user = User.objects.get(pk=user_id)
    ...
    def testUpdateUserRequiredField(self):
        ...
        url = reverse("update_user", kwargs={"version": 3, "user_id": peon.pk})
    ...
    def testUpdateUserNameVersion2(self):
        ...
        url = reverse("update_user", kwargs={"version": 2, "user_id": peon.pk})
    ...
    def testCantRemoveRequiredFields(self):
        ...
        resp = put_json(url, {"username": ""}, self.client, user=peon)
    ...
    def testCanOnlyUpdateLoggedInUser(self):
        ...
        resp = put_json(url, {"password": "whateva"}, self.client, user=grunt)
...
class SigningTest(OTMTestCase):
    ...
    def process_request_through_url(self, req):
        return check_signature(lambda req, *args, **kwargs: req)(req)
    ...
    def testAwsExample(self):
        ...
        self.assertEquals(sig, "i91nKc4PWAt0JJIdXwz9HxZCJDdiy6cf/Mj6vPxyYIs=")

    def testTimestampVoidsSignature(self):
        acred = APIAccessCredential.create()
        url = (
            "http://testserver.com/test/blah?"
            "timestamp=%%s&"
        ...
        req = self.sign_and_send(
            url % invalid.strftime(SIG_TIMESTAMP_FORMAT), acred.secret_key
        )
    ...
    def testMalformedTimestamp(self):
        ...
        req = self.sign_and_send(url % ("%sFAIL" % timestamp), acred.secret_key)
    ...
    def testMissingAccessKey(self):
        ...
        url = "http://testserver.com/test/blah?" "timestamp=%s&" "k1=4&k2=a" % timestamp
        ...
        req = self.sign_and_send(
            "%s&access_key=%s" % (url, acred.access_key), acred.secret_key
        )
    ...
    def testAuthenticatesAsUser(self):
        ...
        req = self.sign_and_send(
            "http://testserver.com/test/blah?"
            "timestamp=%s&"
            "k1=4&k2=a&access_key=%s" % (timestamp, acred.access_key),
            acred.secret_key,
        )
...
class UserApiExportsTest(UserExportsTestCase):
    def _test_requires_admin_access(self, endpoint_name):
        url = reverse(
            "user_csv",
            kwargs={"version": 3, "instance_url_name": self.instance.url_name},
        )
...
class PasswordResetTest(OTMTestCase):
    ...
    def test_send_password_reset_email_url(self):
        ...
        response = post_json(url % (API_PFX, self.jim.email), {}, self.client, None)
        self.assertEquals(response.status_code, 200)
...
class SpeciesListTest(OTMTestCase):
    ...
    def test_species_list_endpoint(self):
        response = get_signed(
            self.client,
            "%s/instance/%s/species" % (API_PFX, self.instance.url_name),
            user=self.user,
        )
        self.assertEqual(response.status_code, 200)
