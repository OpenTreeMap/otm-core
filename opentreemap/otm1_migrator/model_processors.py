...
from otm1_migrator.models import OTM1UserRelic, OTM1ModelRelic, OTM1CommentRelic
from otm1_migrator.data_util import (
    MigrationException,
    sanitize_username,
    uniquify_username,
    inflate_date,
...
@atomic
def save_species(
    migration_rules, migration_event, species_dict, species_obj, instance, **kwargs
):
    ...
    if len(list(non_migrated_species)) > 0:
        raise MigrationException(
            "You cannot migrate species, at all, "
...
@atomic
def save_plot(
    migration_rules, migration_event, plot_dict, plot_obj, instance, **kwargs
):
...
@atomic
def save_tree(
    migration_rules, migration_event, tree_dict, tree_obj, instance, **kwargs
):

    if (
        tree_dict["fields"]["present"] is False
        or tree_dict["fields"]["plot"] == models.UNBOUND_MODEL_ID
    ):
        tree_obj = None
        pk = models.UNBOUND_MODEL_ID
    else:
...
def process_reputation(
    migration_rules, migration_event, model_dict, rep_obj, instance, **kwargs
):

    iuser = InstanceUser.objects.get(
        user_id=model_dict["fields"]["user"], instance_id=instance.id
    )
...
def process_userprofile(
    ...
    * "migrate" userprofile using this migrator
    ...
    try:
        ...
        print(
            "Failed to read photo %s ... SKIPPING USER %s %s"
...
def save_treephoto(
    ...
    **kwargs
    ...
    if model_dict["fields"]["tree"] == models.UNBOUND_MODEL_ID:
        ...
        image = open(os.path.join(treephoto_path, model_dict["fields"]["photo"]))
        treephoto_obj.set_image(image)
        treephoto_obj.map_feature_id = Tree.objects.values_list(
            "plot__id", flat=True
...
def save_audit(
    ...
    **kwargs
    ...
    if fields["field"] == "id" and fields["current_value"] == fields["model_id"]:
...
@atomic
def save_treefavorite(
    migration_rules, migration_event, fav_dict, fav_obj, instance, **kwargs
):
...
def _base_process_comment(
    migration_rules, migration_event, relic_ids, model_dict, comment_obj, instance
):
    ...
    if comment_obj.content_type_id == models.UNBOUND_MODEL_ID:
        try:
            print(
                "Can't import comment %s because "
    ...
    old_object_id = int(
        model_dict["fields"].get("object_pk", model_dict["fields"].get("object_id"))
    )
    try:
        new_object_id = relic_ids[content_type.model][old_object_id]
    except KeyError:
        raise MigrationException(
            "threadedcomment dependency not met. "
            "did you import %s yet?" % comment_obj.content_type.model
        )
    ...
    if new_object_id == models.UNBOUND_MODEL_ID:
        print(
            ...
            % (comment_obj.comment[:10] + "...", content_type.model, old_object_id)
        )
        return None
...
def save_comment(
    ...
    **kwargs
    ...
    comment_obj = _base_process_comment(
        migration_rules, migration_event, relic_ids, model_dict, comment_obj, instance
    )
...
def save_threadedcomment(
    ...
    **kwargs
    ...
    tcomment_obj = _base_process_comment(
        migration_rules, migration_event, relic_ids, model_dict, tcomment_obj, instance
    )
    ...
    if model_dict["fields"]["parent"]:
        parent_relic = OTM1CommentRelic.objects.get(
            instance=instance, otm1_model_id=model_dict["fields"]["parent"]
        )
    ...
    if (
        parent_relic
        and parent_relic.otm1_last_child_id is not None
        and parent_relic.otm1_last_child_id == model_dict["pk"]
    ):
...
@atomic
def process_contenttype(
    migration_rules, migration_event, model_dict, ct_obj, instance, **kwargs
):
    """
    ...
    app_label = "treemap" if fields["model"] == "user" else fields["app_label"]

    try:
        content_type_id = ContentType.objects.get(
            model=fields["model"], app_label=app_label
        ).pk

    except ContentType.DoesNotExist:
        print("SKIPPING ContentType: %s.%s" % (fields["app_label"], fields["model"]))
...
@atomic
def save_boundary(
    migration_rules, migration_event, model_dict, boundary_obj, instance, **kwargs
):
    boundary_obj.save()
    OTM1ModelRelic.objects.create(
        instance=instance,
...
@atomic
def save_registrationprofile(
    migration_rules, migration_event, rp_dict, candidate_rp_obj, instance, **kwargs
):
...
@atomic
def save_user(
    migration_rules, migration_event, user_dict, user_obj, instance, **kwargs
):
    """
    ...
    if users_with_this_email.exists():
        ...
        user_obj = user_obj
        user_obj.username = uniquify_username(sanitize_username(user_obj.username))
        user_obj.save()
    ...
    try:
        ...
        InstanceUser.objects.create(
            instance=instance, user=user_obj, role=instance.default_role
        )
