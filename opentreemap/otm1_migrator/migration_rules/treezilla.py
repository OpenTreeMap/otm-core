...
def set_boundary_fields(boundary_obj, boundary_dict):
    ...
    boundary_type = BOUNDARY_TYPES.get(boundary_dict["fields"].get("city", None), None)
    if boundary_type is None:
        raise MigrationException(
            "boundary_dict missing valid city value: " + str(boundary_dict)
        )
...
MIGRATION_RULES["boundary"]["presave_actions"] = MIGRATION_RULES["boundary"].get(
    "presave_actions", []
) + [set_boundary_fields]
...
MIGRATION_RULES["tree"]["presave_actions"] = MIGRATION_RULES["tree"].get(
    "presave_actions", []
...
def create_override(species_obj, species_dict):
    ...
    if not itree_code:
        raise MigrationException(
            "species_dict missing itree_code: " + str(species_dict)
        )
...
MIGRATION_RULES["species"]["postsave_actions"] = MIGRATION_RULES["species"].get(
    "postsave_actions", []
) + [create_override]
...
UDFS = {
    "plot": {
        ...
        "sidewalk_damage": {
            ...
            "udf.choices": ["Minor or No Damage", "Raised More than 25 mm"],
        },
    },
    "tree": {
