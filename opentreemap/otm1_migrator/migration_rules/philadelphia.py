...
UDFS = {
    "plot": {
        "owner_additional_id": {"udf.name": "Owner Additional Id"},
        "owner_additional_properties": {"udf.name": "Owner Additional Properties"},
        "type": {
            "udf.name": "Plot Type",
            "udf.choices": [
                "Well/Pit",
...
def mutate_boundary(boundary_obj, boundary_dict):
    ...
    if boundary_obj.name.find("County") != -1 or boundary_obj.name == "Philadelphia":
