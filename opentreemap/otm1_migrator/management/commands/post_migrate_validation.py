...
class Command(InstanceDataCommand):
    ...
    def test_class(self, clz, instance_id=None):
        ...
        object_relics = OTM1ModelRelic.objects.filter(otm2_model_name__iexact=name)
        ...
        self.pseudo_assert_equals(
            object_count,
            object_relic_count,
            "there are an equal number of " "%s and %s relics." % (name, name),
        )
