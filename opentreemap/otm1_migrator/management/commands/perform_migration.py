...
from otm1_migrator.models import (
    OTM1UserRelic,
    OTM1ModelRelic,
    OTM1CommentRelic,
    MigrationEvent,
...
from otm1_migrator.data_util import (
    dict_to_model,
    MigrationException,
    add_udfs_to_migration_rules,
    create_udfs,
)

from otm1_migrator import data_util
from otm1_migrator.model_processors import (
    save_boundary,
    save_user,
    save_audit,
    save_species,
...
def save_objects(
    ...
    dicts_to_save = (dict for dict in model_dicts if dict["pk"] not in model_key_map)

    for model_dict in dicts_to_save:
        dependencies = (
            migration_rules.get(model_name, {}).get("dependencies", {}).items()
        )
                ...
                if old_id:
                    ...
                    try:
                        ...
                        raise MigrationException(
                            "Dependency not found. " "Have you imported %s yet?" % name
                        )
                    model_dict["fields"][field] = new_id

        model = dict_to_model(migration_rules, model_name, model_dict, instance)

        if model == data_util.DO_NOT_PROCESS:
            continue
        else:
            model = model_process_fn(
                model_dict, model, instance, old_model_dict=old_model_dict
            )
            if model != data_util.PROCESS_WITHOUT_SAVE and model is not None:
                pk = model.pk
                for fn in migration_rules[model_name].get("postsave_actions", []):
                    fn(model, model_dict)
                if callable(message_receiver):
                    message_receiver("saved model: %s - %s" % (model_name, model.pk))
            else:
                pk = models.UNBOUND_MODEL_ID
            model_key_map[model_dict["pk"]] = pk


from otm1_migrator.migration_rules.standard_otm1 import MIGRATION_RULES as RULES
from otm1_migrator.migration_rules.standard_otm1 import MODEL_ORDER as ORDER
...
class Command(InstanceDataCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.DEBUG:
            self.stdout.write(
                "In order to run this command you must manually"
        ...
        rule_module = (
            options["rule_module"] or "otm1_migrator.migration_rules.standard_otm1"
        )
        ...
        user_photo_fixture_specified_but_not_base_path = (
            userphoto_path is None and options.get("userphoto_fixture") is not None
        )

        if user_photo_fixture_specified_but_not_base_path:
            raise MigrationException(
                "Must specify the user photo path to "
                "import photos. please include a %s or "
                "%s flag when importing." % USERPHOTO_ARGS
            )

        treephoto_path = options.get("treephoto_path", None)
        treephoto_fixture_with_no_path = (
            treephoto_path is None and options.get("treephoto_fixture") is not None
        )

        if treephoto_fixture_with_no_path:
            raise MigrationException(
                "Must specify the tree photo path to " "import photo"
            )
        ...
        process_fns = {
            ...
            "userprofile": default_partial(process_userprofile, userphoto_path),
            "threadedcomment": default_partial(save_threadedcomment, relic_ids),
        ...
        self.stdout.write(_rpad_string(50, ".", "Done reading relics into memory"))
        ...
        def _get_json_dict(model_name):
            ...
            if options[option_name] and os.path.exists(options[option_name]):
                ...
                self.stdout.write(
                    "%sSUCCESS"
                    % _rpad_string(50, ".", "Loaded fixture '%s'" % option_name)
                )
                ...
                self.stdout.write(
                    "%sSKIPPING"
                    % _rpad_string(50, ".", "No valid '%s' fixture " % model_name)
                )
                json_dict = None
            return json_dict
        ...
        for model in model_order:
            ...
            if json_dict:
                ...
                sorted_dicts = sorted(json_dict, key=operator.itemgetter("pk"))
                try:
                    save_objects(
                        migration_rules,
                        model,
