...
function initMarkerPopupStreams() {
    ...
    var renderPopupContent = function(loc, popup, tree) {
        var species = _.findWhere(_species, {otm_code: tree.species()}),
...
function displayValidationErrors(validationErrors) {
    ...
    _.each(groupedErrors, function(errors, message) {
        ...
        var errorLevel = _.first(errors).level || 'error';
        if (errorLevel === 'error') {
        ...
        _.each(
            _.filter(_.flatten(_.map(errors, findValidationErrorField), true)),
            function($el) {
