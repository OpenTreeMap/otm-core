...
class GrowthModelUrbanTreeDatabase(object):
    ...
    def get_default_params(cls, instance):
        return deepcopy({"model_name": "UrbanTreeDatabase", "version": 1, "params": {}})
    ...
    def init_tree(self, tree):
        (age_to_diameter, min_age, max_age) = _growth_data[self._itree_region][
            tree.species.otm_code
        ]
        ...
        if tree.diameter < min_diameter:
            ...
            raise Exception(
                "Initial diameter %s above max %s for %s in %s"
                % (
                    tree.diameter,
                    max_diameter,
                    tree.species.otm_code,
            ...
            tree.initial_age = bisect(
                age_to_diameter, min_age, max_age, tree.diameter, 16, 0.1
            )
...
def loglogw1(a, b, mse):
    return lambda age: exp(a + b * log(log(age + 1) + mse / 2))


def loglogw2(a, b, mse):
    return lambda age: exp(a + b * log(log(age + 1)) + sqrt(age) * mse / 2)


def loglogw3(a, b, mse):
    return lambda age: exp(a + b * log(log(age + 1)) + age * mse / 2)


def loglogw4(a, b, mse):
    return lambda age: exp(a + b * log(log(age + 1)) + age ** 2 * mse / 2)


def lin(a, b):
    return lambda age: a + b * age


def quad(a, b, c):
    return lambda age: a + b * age + c * age ** 2


def cub(a, b, c, d):
    return lambda age: a + b * age + c * age ** 2 + d * age ** 3


def expow1(a, b, mse):
    return lambda age: exp(a + b * age + mse / 2)


def expow2(a, b, mse):
    return lambda age: exp(a + b * age + sqrt(age) * mse / 2)


def expow3(a, b, mse):
    return lambda age: exp(a + b * age + age * mse / 2)


def expow4(a, b, mse):
    return lambda age: exp(a + b * age + age ** 2 * mse / 2)
...
_growth_data = {
    "CaNCCoJBK": {  # NoCalC
        ...
        "ACME": (
            cub(2.85114, -0.12224, 0.05596, -0.0005),
            2.78,
            73.78,
        ),  # max_age lowered
        ...
        "ROPS": (
            cub(2.84813, 0.18509, 0.07077, -0.00096),
            2.85,
            50.85,
        ),  # max_age lowered
        ...
        "ULPA": (
            cub(2.85154, -0.15554, 0.07672, -0.00112),
            2.84,
            44.84,
        ),  # max_age lowered
    },
    "CenFlaXXX": {  # CenFla
        ...
        "JUSI": (
            cub(-0.6109, 1.63311, 0.0145, -0.00022),
            1.04,
            76.04,
        ),  # max_age lowered
        ...
        "QULA2": (
            cub(1.30621, 1.23988, 0.04912, -0.00065),
            1.31,
            61.31,
        ),  # max_age lowered
    ...
    "GulfCoCHS": {  # GulfCo
        ...
        "CELA": (
            cub(3.03874, 0.92424, 0.06529, -0.00125),
            3.04,
            41.04,
        ),  # max_age lowered
        ...
        "ILOP": (
            cub(2.53924, 0.07682, 0.08877, -0.00172),
            2.54,
            34.54,
        ),  # max_age lowered
        ...
        "QUNI": (
            cub(2.53058, 0.94546, 0.0262, -0.00032),
            2.53,
            68.53,
        ),  # max_age lowered
    ...
    "InlEmpCLM": {  # InlEmp
        ...
        "PIBR2": (
            cub(5.28421, -1.37336, 0.41881, -0.01147),
            4.12,
            22.12,
        ),  # max_age lowered
    ...
    "InlValMOD": {  # InlVal
        ...
        "GIBI": (
            cub(2.54104, -0.10418, 0.0531, -0.00052),
            2.49,
            67.49,
        ),  # max_age lowered
    ...
    "InterWABQ": {  # InterW
        ...
        "FRVE": (
            cub(4.61345, 0.39975, 0.05339, -0.00074),
            4.61,
            51.61,
        ),  # max_age lowered
    ...
    "LoMidWXXX": {  # LoMidW
        "ACPL": (
            cub(0.98208, 0.25006, 0.03658, -0.0003),
            0.98,
            84.98,
        ),  # max_age lowered
        ...
        "PODE": (
            cub(-23.6314, 6.13541, -0.13834, 0.00102),
            3.71,
            38.71,
        ),  # max_age lowered
    ...
    "MidWstMSP": {  # MidWst
        ...
        "ACSA1": (
            cub(-1.7914, 1.88949, 0.06576, -0.00121),
            2.24,
            47.24,
        ),  # max_age lowered
        "ACSA2": (
            cub(2.52368, 1.31314, 0.01822, -0.00028),
            2.52,
            66.52,
        ),  # max_age lowered
        "CEOC": (
            cub(3.54797, 1.18435, 0.0522, -0.00081),
            3.55,
            52.55,
        ),  # max_age lowered
        "FRAM": (
            cub(2.83778, 0.75231, 0.15946, -0.00486),
            2.84,
            23.84,
        ),  # max_age lowered
        ...
        "GLTR": (
            cub(4.22795, -0.82484, 0.37243, -0.01051),
            3.77,
            22.77,
        ),  # max_age lowered
        "MA2": (cub(3.23454, 1.60114, -0.0723, 0.00185), 3.23, 69.76),
        "QUPA": (
            cub(3.31385, -0.16442, 0.14156, -0.00245),
            3.29,
            38.29,
        ),  # max_age lowered
        ...
        "TICO": (
            cub(3.31492, 1.20157, 0.13063, -0.00336),
            3.31,
            30.31,
        ),  # max_age lowered
    ...
    "NMtnPrFNL": {  # NMtnPr
        ...
        "FRAM": (
            cub(2.8422, 0.77951, 0.03184, -0.00039),
            2.84,
            64.84,
        ),  # max_age lowered
        ...
        "MA2": (lin(2.48841, 1.15593), 2.49, 60.28),
        "PINI": (
            cub(3.16683, 0.6726, 0.06006, -0.00121),
            3.17,
            38.17,
        ),  # max_age lowered
    ...
    "PacfNWLOG": {  # PacfNW
        ...
        "ACSA2": (
            cub(1.47953, 1.65885, 0.00513, -0.00015),
            1.48,
            73.48,
        ),  # max_age lowered
        ...
        "QURU": (
            cub(0.12978, 1.83846, 0.02001, -0.00036),
            1.99,
            63.99,
        ),  # max_age lowered
        "TIAM": (
            cub(-0.57558, 1.6899, 0.00442, -0.00016),
            1.12,
            69.12,
        ),  # max_age lowered
    ...
    "PiedmtCLT": {  # Piedmt
        ...
        "ACSA2": (
            cub(-1.68001, 2.32536, -0.00094, -0.00023),
            0.64,
            56.64,
        ),  # max_age lowered
        ...
        "QUNI": (
            cub(4.51977, 1.26018, 0.02646, -0.00032),
            4.52,
            73.52,
        ),  # max_age lowered
    ...
    "SacVal": {  # Not an i-Tree region??
        ...
        "QUAG": (
            cub(2.25596, 1.43382, 0.01147, -0.00018),
            2.26,
            77.26,
        ),  # max_age lowered
        "QULO": (
            cub(2.28211, 1.66463, -0.01025, 0.00002),
            2.28,
            133.28,
        ),  # max_age lowered
    ...
    "SWDsrtGDL": {  # SWDsrt
        ...
        "MOAL": (
            cub(0.58767, 1.3122, 0.00991, -0.00014),
            1.91,
            83.91,
        ),  # max_age lowered
    ...
    "TpIntWBOI": {  # TpIntW
        ...
        "ACSA1": (
            cub(4.03735, 0.99779, 0.01328, -0.00011),
            4.04,
            108.04,
        ),  # max_age lowered
        ...
        "ROPS": (
            cub(2.53091, 0.90726, 0.02391, -0.00026),
            2.53,
            76.53,
        ),  # max_age lowered
        "TIAM": (quad(0.17199, 1.56439, -0.00867), 3.27, 70.72),
        "ULPU": (
            cub(-2.208, 1.87431, 0.00437, -0.00016),
            1.56,
            72.56,
        ),  # max_age lowered
    },
