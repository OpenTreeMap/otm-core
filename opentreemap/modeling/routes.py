...
get_boundaries_at_point_view = do(
    login_or_401, json_api_call, modeling_instance_request, get_boundaries_at_point
)


run_model_view = do(login_or_401, json_api_call, modeling_instance_request, run_model)
...
plan_view = do(
    ...
    route(
        ...
        PUT=do(json_api_edit, return_400_if_validation_errors, update_plan),
        DELETE=do(render_template("modeling/partials/plans/plans.html"), delete_plan),
    ),
)
...
plans_view = do(
    ...
    route(
        GET=do(
            render_template("modeling/partials/plans/plans.html"), get_plans_context
        ),
        POST=do(json_api_edit, return_400_if_validation_errors, add_plan),
    ),
)
