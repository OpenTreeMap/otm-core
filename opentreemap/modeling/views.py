...
def get_modeling_context(request, instance):
    ...
    species_for_planting = GrowthAndMortalityModel.get_species_for_planting(instance)
...
def update_plan(request, instance, plan_id):
    try:
        plan = Plan.objects.select_for_update().get(
            instance=instance, pk=plan_id, owner_id=request.user.id
        )
...
def delete_plan(request, instance, plan_id):
    plan = get_object_or_404(
        Plan, instance=instance, pk=plan_id, owner_id=request.user.id
    )
    plan.delete()
...
def _prepare_scenario(scenario, instance, region_code):
    ...
    def prepare_tree(tree):
        ...
        trees = [
            {"species": species, "diameter": diameter,} for i in range(0, tree["count"])
        ]
        return trees
...
def get_boundaries_at_point(request, instance):
    ...
    boundaries = [
        {"name": boundary.name, "geom": boundary.geom.geojson}
        for boundary in boundaries
    ]
...
def _run_model(instance, growth_model, scenario, region_code):
    ...
    eco, err = ecobackend.json_benefits_call(
        "eco_scenario.json", eco_input, post=True, convert_params=False
    )
    if err:
        raise Exception(err)
...
def _cm_to_instance_diameter_units(instance):
    ...
    in_to_instance = storage_to_instance_units_factor(instance, "tree", "diameter")
    cm_to_in = 1 / 2.54
