...
exports.buildSearch = function (elems) {
    ...
    return _.reduce(elems, function (preds, key_and_pred, id) {
        var query = {},
            pred = {},
            childPreds,
            getFirstKey = _.compose(_.first, Object.keys);
        ...
        if (key_and_pred.pred === 'IN') {
            ...
            _(allPreds)
                ...
                .each(function(childPreds, identifier) {
                    var unduplicatedKeys = _(childPreds)
                            .map(getFirstKey)
                            .countBy()
                        ...
                        childQuery = _.reduce(childPreds, function(groupedPred, childPred) {
                                if (_.contains(unduplicatedKeys, getFirstKey(childPred))) {
                                    return $.extend(groupedPred, childPred);
                                }
                                return {};
                            }, {});
            ...
            // [{"tree.id": {"ISNULL": false}},{"tree.id": {"ISNULL": true}},{"tree.name": {"IS": "Joe"}}]
            // {"tree.id": [{"ISNULL": false}, {"ISNULL": true}], "tree.name": [{"IS": "Joe"}]}
        } else {
