...
USER_PROFILE_FIELDS = collections.OrderedDict(
    [
        ...
        (
            "email",
            {"label": _("Email"), "identifier": "user.email", "visibility": "private"},
        ),
        (
            "allow_email_contact",
            {
...
def user_audits(request, username):
    ...
    return get_audits(request.user, instance, request.GET.copy(), user=user, **params)
...
def instance_user(request, instance_url_name, username):
    instance = get_instance_or_404(url_name=instance_url_name)
    url = (
        reverse("user", kwargs={"username": username}) + "?instance_id=%s" % instance.pk
    )
...
def forgot_username(request):
    ...
    if not user_email:
        raise ValidationError({"user.email": [_("Email field is required")]})
    ...
    if len(users) == 1:
        ...
        password_reset_url = request.build_absolute_uri(reverse("auth_password_reset"))

        subject = _("Account Recovery")
        body = render_to_string(
            "treemap/partials/forgot_username_email.txt",
            {"user": user, "password_url": password_reset_url},
        )
...
def resend_activation_email(request):
    ...
    def error(error):
        return render(
            request,
            "treemap/resend_activation_email.html",
...
def user(request, username):
    ...
    audit_dict = get_audits(
        request.user, instance, query_vars, user=user, should_count=True
    )
...
def users(request, instance):
    ...
    users_qs = (
        InstanceUser.objects.filter(instance=instance)
    ...
    if query:
        users_qs = users_qs.filter(user__username__icontains=query).order_by(
