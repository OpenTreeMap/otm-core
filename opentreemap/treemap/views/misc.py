...
def edits(request, instance):
    ...
    return get_audits(request.user, instance, request.GET.copy(), user, **params)


def index(request, instance):
    return HttpResponseRedirect(
        reverse("map", kwargs={"instance_url_name": instance.url_name})
    )
...
def add_map_info_to_context(context, instance):
    all_polygon_types = {
        c.map_feature_type for c in leaf_models_of_class(PolygonalMapFeature)
    }
...
def static_page(request, instance, page):
    ...
    return {"content": static_page.content, "title": static_page.name}
...
def boundary_autocomplete(request, instance):
    ...
    boundaries = instance.boundaries.filter(searchable=True).order_by(
        "sort_order", "name"
    )[:max_items]
...
def species_list(request, instance):
    ...
    species_qs = (
        instance.scope_model(Species)
        .order_by("common_name")
        .values(
            "common_name", "genus", "species", "cultivar", "other_part_of_name", "id"
        )
    )
    ...
    def annotate_species_dict(sdict):
        sci_name = Species.get_scientific_name(
            sdict["genus"],
            sdict["species"],
            sdict["cultivar"],
        ...
        sdict.update(
            {"scientific_name": sci_name, "value": display_name, "tokens": tokens}
        )
...
def public_instances_geojson(request):
    def instance_geojson(instance):
        return {
            ...
            "geometry": {
                "type": "Point",
                "coordinates": [instance.center_lat_lng.x, instance.center_lat_lng.y],
            },
            "properties": {
                "name": instance.name,
                "url": reverse(
...
def error_page(status_code):
    ...
    def inner_fn(request):
        ...
        if request.path.startswith("/api/") or "application/json" in request.META.get(
            "HTTP_ACCEPT", ""
        ):
            response = HttpResponse(
                json.dumps({"status": "Failure", "reason": reasons[status_code]}),
