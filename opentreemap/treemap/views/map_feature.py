...
def map_feature_detail(request, instance, feature_id, should_render=False, edit=False):
    context, partial = _map_feature_detail_context(request, instance, feature_id, edit)
...
def _add_plot_field_groups(context, instance):
    ...
    labels.update({v: k for k, v in context["tree"].scalar_udf_names_and_fields})
    labels.update({v: k for k, v in context["plot"].scalar_udf_names_and_fields})

    def info(group):
        group["fields"] = [
            (field, labels.get(field), templates.get(field, "treemap/field/tr.html"))
            for field in group.get("field_keys", [])
        ]
        group["collection_udfs"] = [
            next(udf for udf in udf_defs(instance) if udf.full_name == udf_name)
            for udf_name in group.get("collection_udf_keys", [])
        ]

        return group

    context["field_groups"] = [info(group) for group in instance.web_detail_fields]


def render_map_feature_detail_partial(request, instance, feature_id, **kwargs):
    context, partial = _map_feature_detail_context(request, instance, feature_id)
    return render(request, partial, context)


def render_map_feature_detail(request, instance, feature_id, **kwargs):
    return map_feature_detail(
        request, instance, feature_id, should_render=True, **kwargs
    )


def context_map_feature_detail(request, instance, feature_id, **kwargs):
    return map_feature_detail(
        request, instance, feature_id, should_render=False, **kwargs
    )


def map_feature_photo_detail(request, instance, feature_id, photo_id):
    feature = get_map_feature_or_404(feature_id, instance)
    photo = get_object_or_404(MapFeaturePhoto, pk=photo_id, map_feature=feature)
...
def update_map_feature(request_dict, user, feature):
    ...
    def value_is_redundant(model, field_name, value):
        ...
        if field_name.startswith("udf:") and value in [[], "[]", "", None]:
    ...
    def set_attr_on_model(model, attr, val):
        field_classname = model._meta.get_field(attr).__class__.__name__
    ...
    for (identifier, value) in request_dict.iteritems():
        ...
        object_name, field = dotted_split(
            identifier, 2, failure_format_string=split_template
        )
        if object_name not in feature_object_names + ["tree"]:
            raise ValueError(split_template % identifier)

        if object_name == "tree" and skip_setting_value_on_tree(
            ...
            tree = (
                tree
                or feature.safe_get_current_tree()
            ...
            raise ValueError("Malformed request - invalid model %s" % object_name)
...
def map_feature_popup(request, instance, feature_id):
    ...
    if instance.canopy_enabled:
        context["boundaries_with_canopy"] = _get_boundaries_with_canopy(
            instance, feature.geom
        )
    return context
...
def canopy_popup(request, instance):
    if instance.canopy_enabled:
        ...
        if result:
            return render(
                request,
                "treemap/partials/canopy_popup.html",
...
def _get_boundaries_with_canopy(instance, point):
    boundaries = (
        instance.boundaries.filter(geom__contains=point)
