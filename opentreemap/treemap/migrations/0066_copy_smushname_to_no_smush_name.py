...
base_conflict_template = ("Uh Oh! user: %s has data in "
                          "%%(no_smush_field)s AND %%(smush_field)s. \n"
                          "Overwriting %%(smush_field)s->%%(no_smush_field)s, "
                          "%%(smush_val)s->%%(no_smush_val)s \n")

class Migration(DataMigration):

    def forwards(self, orm):
        for user in orm['treemap.User'].objects.all():
            base_user_template = base_conflict_template % user.username

            for merge_fields in (('first_name', 'firstname'),
                                 ('last_name', 'lastname')):
                no_smush_field, smush_field = merge_fields
                no_smush_val, smush_val = (getattr(user, no_smush_field),
                                           getattr(user, smush_field))
                if smush_val:
                    if no_smush_val:
