...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        udfs = orm.userdefinedfielddefinition.objects.filter(
            Q(name__startswith=' ') | Q(name__endswith=' '))

        for udf in udfs:
            print('Updating UDF name "%s" to "%s"' % (udf.name, udf.name.strip()))
            udf.name = udf.name.strip()
            udf.save()

        # As it happens our DBs have no UDF names with leading blanks
        # (only trailing blanks) so don't worry about checking for "udf: name"
        perms = orm.fieldpermission.objects.filter(field_name__endswith=' ')

        for perm in perms:
            print('Updating field permission name "%s" to "%s"' % (perm.field_name, perm.field_name.strip()))
            perm.field_name = perm.field_name.strip()
            perm.save()

        audits = orm.audit.objects.filter(field__endswith=' ')

        for audit in audits:
            print('Updating audit field "%s" to "%s"' % (audit.field, audit.field.strip()))
            audit.field = audit.field.strip()
            audit.save()
