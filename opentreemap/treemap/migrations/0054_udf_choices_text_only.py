...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        udf_defs = orm.UserDefinedFieldDefinition.objects.all()

        instance_ids = orm.Instance.objects.values_list('id', flat=True)
        plot_udf_choices_by_instance = {pk: [] for pk in instance_ids}
        tree_udf_choices_by_instance = {pk: [] for pk in instance_ids}

        for udf_def in udf_defs:
            datatype_dict = json.loads(udf_def.datatype)
            if 'choices' not in datatype_dict:
                continue

            datatype_dict['choices'] = [str(choice)
                                        for choice in datatype_dict['choices']]
            udf_def.datatype = json.dumps(datatype_dict)
            udf_def.save()

            if udf_def.model_type == "Plot":
                plot_udf_choices_by_instance[udf_def.instance_id] += udf_def.name
            elif udf_def.model_type == "Tree":
                tree_udf_choices_by_instance[udf_def.instance_id] += udf_def.name
