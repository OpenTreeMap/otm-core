...
def add_permission(apps, schema_editor):
    ...
    model_untracked = {
        Model: _model_do_not_track(apps, Model) for Model in authorizable_models
    }
...
def _add_permissions_to_role(
    ...
    for Model in authorizable_models:
        ...
        add_permission, delete_permission = tuple(_get_model_perms(Model, ContentType))
...
def remove_permission(apps, schema_editor):
    ...
    role_add_models = (
        _get_role_model_action_perms(apps, rpms, non_photo_permissions, "add")
        - role_delete_models
    )

    artificial_permissions = Permission.objects.filter(
        content_type__in=ContentType.objects.filter(photo_query)
    ).exclude(codename__endswith="mapfeaturephoto")

    artificial_permissions_roles = {
        ar.role for ar in rpms.filter(permission__in=artificial_permissions)
    }
    ...
    model_untracked = {
        Model: _model_do_not_track(apps, Model)
        for Model in authorizable_models | {MapFeaturePhoto}
    }
    _restore_add_delete_field_permissions(
        FieldPermission, model_untracked, role_add_models, role_delete_models
    )
...
def _restore_photo_permissions(
    FieldPermission, MapFeaturePhoto, model_untracked, artificial_permissions, roles
):
    _restore_add_delete_field_permissions(
        FieldPermission, model_untracked, set(), {(r, MapFeaturePhoto) for r in roles}
    )


def _restore_add_delete_field_permissions(
    FieldPermission, model_untracked, role_add_models, role_delete_models
):
    perms_to_recreate = []
    perms_to_update = []
...
def _get_role_model_action_perms(apps, role_model_permissions, permissions, action):
    ...
    get_model = lambda rmap: apps.get_model(
        rmap.permission.content_type.app_label, rmap.permission.content_type.model
    )
    return {(rmap.role, get_model(rmap)) for rmap in role_model_action_perms}


def _get_model_perms(Model, ContentType):
    return (
        ContentType.objects.get_for_model(Model)
        .permission_set.exclude(codename__startswith="change_")
        .order_by("codename")
    )
...
def _can_create_or_delete(not_tracked, role, Model):
    field_permissions = role.fieldpermission_set.filter(
        model_name=Model.__name__, permission_level=_WRITE_DIRECTLY
    )
    ...
    can_delete_permissions = field_permissions.filter(field_name__in=tracked_fields)
...
def _model_do_not_track(apps, Model):
    ...
    if "udfs" in [f.name for f in Model._meta.fields]:
        no_track |= {"udfs"} | {"udf:{}".format(udfd.name) for udfd in collection_udfs}
    return no_track
...
def _fieldnames_required_for_create(not_tracked, Model):
    return {
        ...
        if (
            ...
            and not field.name in not_tracked
        )
    }


def _tracked_fields(not_tracked, Model):
    return {field.name for field in Model._meta.fields if field.name not in not_tracked}
