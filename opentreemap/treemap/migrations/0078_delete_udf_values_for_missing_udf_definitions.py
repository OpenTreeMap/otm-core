...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for instance in orm.Instance.objects.all():
            for Model, model_name in ((orm.Plot, 'Plot'), (orm.Tree, 'Tree')):

                udf_def_names = set(orm.UserDefinedFieldDefinition.objects
                                    .filter(instance=instance)
                                    .filter(model_type=model_name)
                                    .values_list('name', flat=True)
                                    )

                things_with_udfs = Model.objects.filter(instance=instance) \
                                                .filter(udfs__isnull=False) \
                                                .exclude(udfs="")

                if len(things_with_udfs) == 0:
                    continue

                keys_to_delete = set()
                for thing in things_with_udfs:
                    keys = set(thing.udfs.keys())
                    keys_to_delete |= set(keys - udf_def_names)

                if len(keys_to_delete) == 0:
                    continue

                # We would run the following if we could use the HStore manager:
                # things_with_udfs.hremove('udfs', list(keys_to_delete))

                db.execute("UPDATE %s SET udfs = delete(udfs, %%s) WHERE instance_id = %%s"
                           % Model._meta.db_table, [list(keys_to_delete), instance.pk])
