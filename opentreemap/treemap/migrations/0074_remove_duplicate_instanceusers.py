...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Delete any duplicate instance<->user pairs that snuck in before we set the constraint
        # Keep the oldest InstanceUser row
        db.execute('DELETE FROM treemap_instanceuser WHERE id IN (SELECT id FROM (SELECT id, rank() OVER (PARTITION BY instance_id, user_id ORDER BY id) FROM treemap_instanceuser) tmp WHERE rank > 1);')
