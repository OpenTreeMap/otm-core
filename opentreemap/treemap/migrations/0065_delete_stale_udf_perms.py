...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        udf_defs = orm['treemap.UserDefinedFieldDefinition'].objects.all()
        field_perms = (orm['treemap.FieldPermission']
                       .objects
                       .filter(field_name__startswith='udf:'))
        for perm in field_perms:
            if not udf_defs.filter(model_type=perm.model_name,
                                   name=perm.field_name[4:],
                                   instance=perm.instance).exists():
                perm.delete()
