...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Change treePhoto search to mapFeaturePhoto search
        for instance in orm.Instance.objects.all():
            if 'mobile_search_fields' in instance.config:
                ...
                missing_fields = fields.get('missing', [])
                for field in missing_fields:
                    if field.get('identifier') == 'treePhoto.id':
                        field['identifier'] = 'mapFeaturePhoto.id'
                instance.save()
    ...
    def backwards(self, orm):
        for instance in orm.Instance.objects.all():
            if 'mobile_search_fields' in instance.config:
                ...
                missing_fields = fields.get('missing', [])
                for field in missing_fields:
                    if field.get('identifier') == 'mapFeaturePhoto.id':
                        field['identifier'] = 'treePhoto.id'
                instance.save()
