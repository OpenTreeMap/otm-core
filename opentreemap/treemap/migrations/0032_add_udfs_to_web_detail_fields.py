...
def add_udfs_to_web_detail_fields(apps, schema_editor):
    ...
    for instance in Instance.objects.all():
        scalar_udfs = UserDefinedFieldDefinition.objects.filter(
            iscollection=False, instance=instance
        ).order_by("name")
        ...
        for udf in scalar_udfs:
            for group in instance.config["web_detail_fields"]:
                if "model" in group and group["model"] == to_object_name(
                    udf.model_type
                ):
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(add_udfs_to_web_detail_fields, migrations.RunPython.noop),
    ]
