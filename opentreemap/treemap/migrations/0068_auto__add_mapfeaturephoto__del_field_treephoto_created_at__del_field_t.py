...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'MapFeaturePhoto'
        db.create_table(u'treemap_mapfeaturephoto', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('map_feature', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['treemap.MapFeature'])),
        ...
        db.send_create_signal(u'treemap', ['MapFeaturePhoto'])

        # Manual data copying and fixing of the auto-increment counter
        db.execute("""
INSERT INTO treemap_mapfeaturephoto
    ( id, map_feature_id, image, thumbnail, created_at, instance_id )
SELECT photo.id, tree.plot_id, photo.image, photo.thumbnail, photo.created_at, photo.instance_id
FROM treemap_treephoto AS photo
JOIN treemap_tree AS tree
ON tree.id = photo.tree_id;
        """)

        db.execute("select setval('treemap_mapfeaturephoto_id_seq', (select max(id) from treemap_treephoto));")

        # Deleting field 'TreePhoto.created_at'
        db.delete_column(u'treemap_treephoto', 'created_at')

        # Deleting field 'TreePhoto.thumbnail'
        db.delete_column(u'treemap_treephoto', 'thumbnail')

        # Deleting field 'TreePhoto.instance'
        db.delete_column(u'treemap_treephoto', 'instance_id')

        # Deleting field 'TreePhoto.image'
        db.delete_column(u'treemap_treephoto', 'image')

        # Renaming field 'TreePhoto.id' -> 'TreePhoto.mapfeaturephoto_ptr_id'
        db.rename_column(u'treemap_treephoto', u'id', u'mapfeaturephoto_ptr_id')
        db.alter_column(u'treemap_treephoto', u'mapfeaturephoto_ptr_id',
                        self.gf('django.db.models.fields.related.OneToOneField')(default=0, to=orm['treemap.MapFeaturePhoto'], unique=True, primary_key=True))


    def backwards(self, orm):
        raise Exception('No backwards migrations!')
