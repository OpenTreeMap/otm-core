...
def set_empty_multichoice_values_to_none(apps, schema_editor):
    ...
    udfds = UserDefinedFieldDefinition.objects.filter(datatype__contains="multichoice")
    print()
    for udfd in udfds:
        ...
        objs = Model.objects.filter(instance=udfd.instance).filter(
            udfs__contains={udfd.name: "[]"}
        )
        for obj in objs:
            # Note: this is doing
        ...
        if len(objs) > 0:
            print(
                'Updated %s empty multichoice values for %s udf "%s" (%s)'
                % (len(objs), udfd.model_type, udfd.name, udfd.instance.url_name)
            )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            set_empty_multichoice_values_to_none, migrations.RunPython.noop
        )
    ]
