...
def get_external_link_choice_pattern():
    return "|".join([choice.replace(".", r"\.") for choice in _valid_url_tokens])
...
def _re_group_count(compiled, text):
    ...
    return reduce(
        lambda ac, groupdict: {
            k: v if groupdict[k] is None else v + 1 for k, v in ac.iteritems()
        },
...
def get_url_tokens_for_display(in_bold=False):
    ...
    return (
        ", ".join(show(token) for token in _valid_url_tokens[:-1])
        + unicode(_(" or "))
        + show(_valid_url_tokens[-1])
    )
