...
def _instance_ids_edited_by(user):
    return (
        Audit.objects.filter(user=user)
...
def get_audits(
    ...
    for inst in instances:
        eligible_models = (
            {"Tree", "TreePhoto", "MapFeaturePhoto"} | set(inst.map_feature_types)
        ) & set(models)
        ...
        if logged_in_user == user:
            ...
            for model in eligible_models:
                ...
                if issubclass(ModelClass, Authorizable):
                    ...
                    model_filter = model_filter | Q(
                        model=model, field__in=visible_fields, instance=inst
                    )
                else:
                    model_filter = model_filter | Q(model=model, instance=inst)

                if issubclass(ModelClass, UDFModel):
                    model_collection_udfs_audit_names = fake_model.visible_collection_udfs_audit_names(
                        logged_in_user
                    )
    ...
    udf_bookkeeping_fields = Q(
        model__startswith="udf:", field__in=("id", "model_id", "field_definition")
    )

    audits = (
        Audit.objects.filter(model_filter)
...
def get_audits_params(request):
    ...
    if model_id is not None and len(models) != 1:
        raise Exception("You must specific one and only model " "when looking up by id")

    exclude_pending = r.get("exclude_pending", "false") == "true"

    return {
        "start_id": start_id,
        "prev_start_ids": prev_start_ids,
        "page_size": page_size,
        "models": models,
        "model_id": model_id,
...
def get_user_instances(logged_in_user, user, current_instance=None):
    ...
    user_instance_ids = InstanceUser.objects.filter(user_id=user.pk).values_list(
    ...
    return Instance.objects.filter(instance_filter).distinct().order_by("name")
