...
def _map_feature_audits(user, instance, feature, filters=None, cudf_filters=None):
    ...
    feature_filter = Q(
        model=feature.feature_type, model_id=feature.pk, field__in=readable_plot_fields
    )
    ...
    iaudit = Audit.objects.filter(instance=instance).exclude(user=User.system_user())
    ...
    udf_collection_exclude_filter = Q(field__in=["model_id", "field_definition"])

    for afilter in cudf_filters:
        audits += list(
            iaudit.filter(afilter)
...
def _add_eco_benefits_to_context_dict(instance, feature, context):
    ...
    benefits, basis, failure_code = FeatureClass.benefits.benefits_for_object(
        instance, feature
    )
...
def _plot_audits(user, instance, plot):
    ...
    tree_filter = Q(
        model="Tree", field__in=tree_visible_fields, model_id__in=tree_history
    )

    tree_delete_filter = Q(
        model="Tree", action=Audit.Type.Delete, model_id__in=tree_history
    )

    tree_collection_udfs_audit_names = fake_tree.visible_collection_udfs_audit_names(
        user
    )
...
def context_dict_for_plot(request, plot, tree_id=None, **kwargs):
    ...
    if tree_id:
        tree = get_object_or_404(Tree, instance=instance, plot=plot, pk=tree_id)
    else:
    ...
    def get_external_link_url(user, feature, tree=None):
        ...
        if (
            not external_link_config
            or not external_link_config.get("url", None)
            or not external_link_config.get("text", None)
        ):
            return None
    ...
    has_tree_species_with_code = (
        tree is not None
        and tree.species is not None
        and tree.species.otm_code is not None
    )
    ...
    url_kwargs = {"instance_url_name": instance.url_name, "feature_id": plot.pk}
    if tree:
...
def context_dict_for_resource(request, resource, **kwargs):
    ...
    context["upload_photo_endpoint"] = reverse(
        ...
        kwargs={"instance_url_name": instance.url_name, "feature_id": resource.pk},
    )

    context["progress_percent"] = int(
        100 * (completed_progress_items / total_progress_items) + 0.5
    )
    ...
    if isinstance(resource, PolygonalMapFeature):
        ...
        __, display_area = get_display_value(
            instance, "greenInfrastructure", "area", area, digits=0
        )
...
def context_dict_for_map_feature(request, feature, edit=False):
    ...
    if edit:
        if feature.is_plot or getattr(feature, "is_editable", False):
            ...
            raise PermissionDenied("Cannot edit '%s' objects" % feature.feature_type)
    ...
    if user and user.is_authenticated():
        favorited = Favorite.objects.filter(map_feature=feature, user=user).exists()
    else:
        favorited = False
...
def _add_share_context(context, request, photos):
    ...
    if context.get("benefits_total_currency", 0) > 0:
        description = _("This %(feature)s saves %(currency)s%(amount)s per year.") % {
            "feature": context["title"],
            "currency": context["currency_symbol"],
            "amount": number_format(context["benefits_total_currency"], decimal_pos=0),
        }
    else:
...
def set_map_feature_updated_at():
    models = [Model.map_feature_type for Model in leaf_models_of_class(MapFeature)]
    if not models:
...
FROM (
  ...
  GROUP BY model_id
    ...
    WHERE a.id = treemap_mapfeature.id;"""
        % models_in
    )
...
FROM (
  ...
  ORDER BY a.model_id, updated_at DESC
    ...
    WHERE m.model_id = treemap_mapfeature.id;"""
        % models_in
    )
