...
DEFAULT_SEARCH_FIELDS = DotDict(
    {
        ...
        "Plot": [{"identifier": "plot.owner_orig_id"}],
        "Tree": [{"identifier": "tree.diameter"}, {"identifier": "tree.date_planted"}],
    }
)
...
INSTANCE_FIELD_ERRORS = {
    ...
    "group_has_no_header": _("Every mobile field group must have a non-empty header"),
    "group_has_no_keys": _(
    ...
    "group_has_no_sort_key": _(
        'Collection field groups must have a non-empty "sort_key" property ' "defined"
    ),
    "group_has_missing_cudf": _(
        "Collection field groups can only contain existing custom collection " "fields"
    ),
...
def advanced_search_fields(instance, user):
    ...
    fields = {
        category: get_visible_fields(field_infos, user)
    ...
    fields["display"] = [make_display_filter("Tree"), make_display_filter("Plot")]
    fields["display"] += [
        make_display_filter(feature)
        for feature in sorted(instance.map_feature_types)
        if feature != "Plot"
    ]
...
def mobile_search_fields(instance):
    from treemap.templatetags.form_extras import (
