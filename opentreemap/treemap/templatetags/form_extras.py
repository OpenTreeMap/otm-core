...
from treemap.units import (
    get_digits_if_formattable,
    get_units_if_convertible,
...
_identifier_regex = re.compile(r"^[a-zA-Z_.\-]+(?:udf\:.+|[a-zA-Z0-9_\-]+)$")
...
class Label(Grammar):
    grammar = G(b'_("', WORD(b'^"'), b'")') | G(b"_('", WORD(b"^'"), b"')") | Variable


class InlineEditGrammar(Grammar):
    grammar = (
        OR(G(OR(b"field", b"create"), OPTIONAL(Label)), b"search"),
        b"from",
        Variable,
        OPTIONAL(b"for", Variable),
        OPTIONAL(b"in", Variable),
        b"withtemplate",
        Variable,
...
def inline_edit_tag(tag, Node):
    ...
    def tag_parser(parser, token):
        ...
        label = _token_to_variable(
            elems[0][1].string if len(elems[0].elements) > 1 and elems[0][1] else None
        )
        ...
        return Node(label, identifier, user, field_template, instance, help_text)
...
def _is_udf(model, udf_field_name):
    return hasattr(model, "udf_field_names") and udf_field_name in model.udf_field_names
...
def field_type_label_choices(
    model, field_name, label=None, explanation=None, add_blank=ADD_BLANK_IF_CHOICE_FIELD
):
    choices = None
    ...
    if not _is_udf(model, udf_field_name):
        ...
        try:
            ...
            raise Exception(
                "This template tag only supports %s not %s"
                % (VALID_FIELD_KEYS, field_type)
            )
        label = label if label else field.verbose_name
        explanation = explanation if explanation else field.help_text
        choices = [
            {"value": choice[0], "display_value": choice[1]} for choice in field.choices
        ]
        ...
        if "choices" in udf_dict:
            choices = [
                {"value": value, "display_value": value}
                for value in udf_dict["choices"]
            ]
            if add_blank == ADD_BLANK_ALWAYS or (
                add_blank == ADD_BLANK_IF_CHOICE_FIELD and field_type == "choice"
            ):
...
class AbstractNode(template.Node):
    def __init__(self, label, identifier, user, field_template, instance, explanation):
        self.label = label
    ...
    def render(self, context):
        ...
        field_template = get_template(
            _resolve_variable(self.field_template, context)
        ).template

        if not isinstance(identifier, basestring) or not _identifier_regex.match(
            identifier
        ):
            raise template.TemplateSyntaxError(
        ...
        if is_json_field_reference(field_name):
            ...
            add_blank = (
                ADD_BLANK_ALWAYS
                if self.treat_multichoice_as_choice
                else ADD_BLANK_IF_CHOICE_FIELD
            )
            data_type, label, explanation, choices = field_type_label_choices(
                model, field_name, label, explanation=explanation, add_blank=add_blank
            )
...
class SearchNode(CreateNode):
    def __init__(self, __, identifier, user, template, instance, explanation):
        super(SearchNode, self).__init__(
            None, None, user, template, instance, explanation
        )
    ...
    def _fill_in_typeahead(self, field, field_name, model, search_query):
        def get_search_query_value(
            column, display, identifier, related_class, search_query
        ):
            if not search_query:
                return ""
            search_map = json.loads(search_query) or {}
            model_name, field_name = tuple(dotted_split(identifier, 2, maxsplit=1))
        ...
        relation_lookup_infos = {
            "treemap.models.User": {
                ...
                "url": reverse(
                    "users", kwargs={"instance_url_name": model.instance.url_name}
                )
                + "?q=%Q%",
        ...
        model_name = "{}.{}".format(related_model.__module__, related_model.__name__)
