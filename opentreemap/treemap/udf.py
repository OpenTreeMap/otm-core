...
class UserDefinedFieldDefinition(models.Model):
    ...
    objects. Each dictionary object has the following required keys:
    ...
    def from_string_to_python(self, value):
        ...
        if self.datatype_dict[0]['type'] == 'float':
            try:
                return float(value)
            except ValueError:
                raise ValidationError(_('%(fieldname)s '
                                      ...
                                      {'fieldname': self.name})
        elif self.datatype_dict[0]['type'] == 'int':
            try:
                return int(value)
            except ValueError:
                raise ValidationError(_('%(fieldname)s '
                                        'must be an integer') %
                                      {'fieldname': self.name})
        elif self.datatype_dict[0]['type'] == 'user':
            return User.objects.get(pk=value)
        elif self.datatype_dict[0]['type'] == 'date':
            return datetime.strptime(value, '%Y%m%d%H%M%S')
        elif self.datatype_dict[0]['type'] == 'choice':
            if value in self.datatype_dict[0]['choices']:
                return value
            else:
    ...
    def from_python_to_string(self, value):
        ...
        # We only support scalar UDFs right now
        # so we can just grab the first one
        if self.datatype_dict[0]['type'] == 'user':
...
class UDFModel(UserTrackable, models.Model):
    ...
    def apply_change(self, key, oldval):
        if key in [field.name for field in self.get_user_defined_fields()]:
            ...
            super(UDFModel, self).revert_change(key, oldval)
