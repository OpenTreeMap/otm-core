...
from treemap.units import (
    is_convertible,
    is_convertible_or_formattable,
    get_display_value,
    get_units,
    get_unit_name,
    Convertible,
...
def _add_default_add_and_delete_permissions(models, roles):
    ...
    role_perms = [
        ThroughModel(role_id=role_id, permission_id=perm.id)
...
def _add_default_field_permissions(models, role, instance):
    ...
    if existing.exists():
        for perm in perms:
            perm["defaults"] = {"permission_level": role.default_permission_level}
    ...
    if approved:
        ...
        try:
            ...
            if audit.field == "id":
                ...
                most_recent_audits = Audit.objects.filter(
                    model=audit.model,
                    model_id=audit.model_id,
                    instance=audit.instance,
                    field=audit.field,
                ...
                try:
                    ...
                    if is_most_recent_audit:
                        obj.apply_change(audit.field, audit.clean_previous_value)
...
def approve_or_reject_audit_and_apply(audit, user, approved):
    ...
    if approved:
        ...
        try:
            obj = TheModel.objects.get(pk=audit.model_id)
            obj.apply_change(audit.field, audit.clean_current_value)
...
def _process_approved_pending_insert(model_class, user, audit):
    ...
    for approved_audit in approved_audits:
        obj.apply_change(approved_audit.field, approved_audit.clean_current_value)
...
def get_related_audits(insert_audit, approved_only=False):
    ...
    """
    related_audits = Audit.objects.filter(
        instance=insert_audit.instance,
        model_id=insert_audit.model_id,
        model=insert_audit.model,
        action=Audit.Type.Insert,
    ).exclude(pk=insert_audit.pk)
    if approved_only:
        related_audits = related_audits.filter(ref__action=Audit.Type.PendingApprove)
...
def _verify_user_can_apply_audit(audit, user):
    ...
    perms = [
        perm
        for perm in field_permissions(user, audit.instance, model)
        if perm.field_name == field
    ]
    if len(perms) == 1:
        if perms[0].permission_level != FieldPermission.WRITE_DIRECTLY:
            raise AuthorizeException(
                "User %s can't edit field %s on model %s" % (user, field, model)
            )
    elif len(perms) == 0:
        raise AuthorizeException(
            "User %s can't edit field %s on model %s"
            " (No permissions found)" % (user, field, model)
        )
...
def _get_model_and_field(audit):
    if audit.model.startswith("udf:"):
        ...
        udf = [defn for defn in udf_defs(audit.instance) if defn.pk == key][0]
...
class UserTrackable(Dictable):
    ...
    def _direct_updates(self, updates, user):
        ...
        return {
            key: val for key, val in updates.iteritems() if key not in pending_fields
        }

    def _pending_updates(self, updates, user):
        pending_fields = self.get_pending_fields(user)
        return {key: val for key, val in updates.iteritems() if key in pending_fields}
    ...
    def clean(self):
        try:
            ...
            raise ValidationError(
                "Model '%s' does not exist or is not Authorizable." % self.model_name
            )
        except FieldDoesNotExist as e:
            raise ValidationError(
                "%s: Model '%s' does not have field '%s'"
                % (e.__class__.__name__, self.model_name, self.field_name)
            )
        except AssertionError as e:
            raise ValidationError(
                "%s: '%s' is not an Authorizable model. "
                "FieldPermissions can only be set "
                "on Authorizable models." % (e.__class__.__name__, self.model_name)
            )
...
class Role(models.Model):
    ...
    @classmethod
    def model_permissions(clz, models, actions=None, include_photos=True):
        if actions is None:
            actions = ["add", "delete"]
        perm_names = [
            Role.permission_codename(Model, action)
            for Model in models
...
class Authorizable(UserTrackable):
    ...
    def user_can_delete(self, user):
        can_delete = user.get_role(self.get_instance()).can_delete(self.__class__)
        if not can_delete:
    ...
    def get_pending_fields(self, user):
        ...
        for perm in perms:
            if (
                perm.permission_level == FieldPermission.WRITE_WITH_AUDIT
                and perm.field_name in tracked_fields
                and perm.field_name not in self.bypasses_authorization
            ):
    ...
    def save_with_user(self, user, *args, **kwargs):
        ...
        if self.pk is not None:
            for field in self._updated_fields():
                if field not in self._get_writable_perms_set(user):
                    raise AuthorizeException(
                        "Can't edit field %s on %s" % (field, self._model_name)
                    )
        ...
        # and making the audit objects required for approval or rejection.
            ...
            raise AuthorizeException(
                ...
                "create new %s objects." % (user, self._model_name)
            )
    ...
    def save_with_system_user_bypass_auth(self, *args, **kwargs):
        ...
        super(Authorizable, self).save_with_user(
            User.system_user(), auth_bypass=True, *args, **kwargs
        )
    ...
    def delete_with_user(self, user, *args, **kwargs):
        ...
        if self.user_can_delete(user):
            ...
            raise AuthorizeException(
                "%s does not have permission to "
                "delete %s %s." % (user, self._model_name, self.id)
            )
    ...
    def requires_authorization(cls):
        ...
        """
        return {
            f.name for f in cls._meta.fields if f.name not in cls.bypasses_authorization
        }
...
class Auditable(UserTrackable):
                ...
                try:
                    ...
                    if related_model is not None:
                        ...
                        raise IntegrityError(
                            "%s has null required field %s" % (self, field.name)
                        )
                except ObjectDoesNotExist:
                    raise IntegrityError("%s has non-existent %s" % (self, field.name))
    ...
    def hash(self):
        ...
        audits = (
            Audit.objects.filter(model=self._model_name)
    ...
    def action_format_string_for_audit(clz, audit):
        if audit.field == "id" or audit.field is None:
            lang = {
                ...
                Audit.Type.PendingApprove: _("approved an " "edit to the %(model)s"),
                Audit.Type.PendingReject: _("rejected an " "edit to the %(model)s"),
            }
        else:
            lang = {
                ...
                Audit.Type.PendingApprove: _(
                    "approved setting " "%(field)s to %(value)s"
                ),
                Audit.Type.PendingReject: _(
                    "rejecting setting " "%(field)s to %(value)s"
                ),
            }
...
class _PendingAuditable(Auditable):
    ...
    def save_with_user(self, user, auth_bypass=False, *args, **kwargs):
        ...
        if self.user_can_create(user) or not is_insert or auth_bypass:
            ...
            model_id = _reserve_model_id(get_authorizable_class(self._model_name))
            self.pk = model_id
...
class Audit(models.Model):
    ...
    instance = models.ForeignKey("Instance", null=True, blank=True, db_index=True)
    ...
    class Meta:
        index_together = [["instance", "user", "updated"]]
    ...
    def _deserialize_value(self, value):
        ...
        if self.field.startswith("udf:"):
            ...
            if self.model.startswith("udf:"):
                ...
                udf_def = next((udfd for udfd in udfds if udfd.pk == udfd_pk), None)
                ...
                udf_def = next(
                    (
                        udfd
                        for udfd in udfds
                        if udfd.name == field_name and udfd.model_type == self.model
                    ),
                    None,
                )
                if udf_def is not None:
        ...
        if isinstance(field_cls, models.GeometryField):
            ...
            if isinstance(field_modified_value, (str, unicode)):
                ...
                try:
                    ...
                    field_modified_value = field_cls.rel.to.objects.get(pk=pk)
                except ValueError:
                    pass
    ...
    def _unit_format(self, value):
        ...
        if is_convertible_or_formattable(model_name, field):
            __, value = get_display_value(self.instance, model_name, field, value)
            if value and is_convertible(model_name, field):
                units = get_unit_name(get_units(self.instance, model_name, field))
                value += " %s" % units
    ...
    def audits_for_model(clz, model_name, inst, pk):
        return Audit.objects.filter(
            model=model_name, model_id=pk, instance=inst
        ).order_by("created")

    @classmethod
    def pending_audits(clz):
        return (
            Audit.objects.filter(requires_auth=True)
            .filter(ref__isnull=True)
            .order_by("created")
        )
...
class ReputationMetric(models.Model):
    ...
    def apply_adjustment(*audits):
        ...
        for audit in audits:
            try:
                rm = ReputationMetric.objects.get(
                    instance=audit.instance, model_name=audit.model, action=audit.action
                )
            except ObjectDoesNotExist:
                return
            ...
            if audit.requires_auth and audit.ref:
                ...
                if review_audit.action == Audit.Type.PendingApprove:
                    ...
                    error_message = (
                        "Referenced Audits must carry approval "
...
get_authorizable_class = partial(_get_model_class, _authorizable_classes, Authorizable)
