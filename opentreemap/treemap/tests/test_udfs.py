...
def make_collection_udf(instance, name="Stewardship", model="Plot", datatype=None):
    # Need to setup the hstore extension to make UDFs
    psycopg2.extras.register_hstore(connection.cursor(), globally=True)

    if datatype is None:
        datatype = [
            {"type": "choice", "choices": ["water", "prune"], "name": "action"},
...
class ScalarUDFTestCase(OTMTestCase):
    def setUp(self):
        ...
        UserDefinedFieldDefinition.objects.create(
            ...
            model_type="Plot",
            datatype=json.dumps(
                {"type": "multichoice", "choices": ["a", "contains a", "also does"]}
            ),
            iscollection=False,
...
class ScalarUDFFilterTest(ScalarUDFTestCase):
    ...
    def test_filtering_on_string_and_choice_using_count(self):
        ...
        self.assertEqual(len(self.choice_a), plots.count())

    def test_filtering_on_value_works(self):
        plots = Plot.objects.filter(**{"udfs__Test choice": "b"})
        self.assertEqual(self.choice_b, {plot.pk for plot in plots})
    ...
    def test_combine_with_geom(self):
        ...
        a_in_poly = Plot.objects.filter(**{"udfs__Test choice": "a"}).filter(
            geom__contained=poly
        )

        self.assertEqual({plot.pk for plot in a_in_poly}, {plot_a.pk,})

        b_in_poly = Plot.objects.filter(**{"udfs__Test choice": "b"}).filter(
            geom__contained=poly
        )

        self.assertEqual({plot.pk for plot in b_in_poly}, {plot_b.pk,})
    ...
    def test_search_suffixes(self):
        ...
        def run(sfx, val):
            return {
                plot.pk
                for plot in Plot.objects.filter(**{"udfs__Test string" + sfx: val})
            }
    ...
    def test_integer_gt_and_lte_constraints(self):
        ...
        plots = Plot.objects.filter(
            **{"udfs__Test int__int__gt": 20, "udfs__Test int__int__lte": 50}
        )
    ...
    def test_using_q_objects(self):
        ...
        self.assertEqual(self.choice_b | self.choice_c, {plot.pk for plot in plots})
...
class UDFAuditTest(OTMTestCase):
    def setUp(self):
        ...
        self.commander_user = make_commander_user(self.instance)
        set_write_permissions(
            self.instance, self.commander_user, "Plot", ["udf:Test choice"]
        )
    ...
    def test_cant_edit_unauthorized_collection(self):
        ...
        self.assertRaises(
            AuthorizeException, self.plot.save_with_user, self.commander_user
        )
    ...
    def test_create_invalid_pending_collection(self):
        ...
        self.assertRaises(
            ValidationError, self.plot.save_with_user, self.commander_user
        )
...
class UDFDefTest(OTMTestCase):
    ...
    def test_cannot_create_duplicate_udfs(self):
        self._create_and_save_with_datatype({"type": "string"}, name="random")
        ...
        self._create_and_save_with_datatype({"type": "string"}, name="random2")
    ...
    def test_must_have_type_key(self):
        self.assertRaises(ValidationError, self._create_and_save_with_datatype, {})

    def test_invalid_type(self):
        self.assertRaises(
            ValidationError, self._create_and_save_with_datatype, {"type": "woohoo"}
        )
    ...
    def test_description_op(self):
        self._create_and_save_with_datatype(
            {"type": "float", "description": "this is a float field"}
        )

    def test_choices_not_missing(self):
        self.assertRaises(
            ValidationError, self._create_and_save_with_datatype, {"type": "choice"}
        )

        self._create_and_save_with_datatype(
            {"type": "choice", "choices": ["a choice", "another"]}
        )
    ...
    def test_cannot_create_choices_with_numeric_values(self):
        with self.assertRaises(ValidationError):
            self._create_and_save_with_datatype(
                {"type": "choice", "choices": [0, 1, 3, 4, 5]}
            )

    def test_can_create_subfields(self):
        self._create_and_save_with_datatype(
            [
                {"type": "choice", "name": "achoice", "choices": ["a", "b"]},
                {"type": "string", "name": "something"},
            ],
            iscollection=True,
        )
    ...
    def test_must_have_name_on_subfields(self):
        ...
        self._create_and_save_with_datatype(
            [
                {"type": "choice", "name": "valid name", "choices": ["a", "b"]},
                {"type": "string", "name": "something"},
            ],
            iscollection=True,
        )
    ...
    def test_iscollection_requires_json_array(self):
        self.assertRaises(
            ...
            self._create_and_save_with_datatype,
            [
                {"type": "choice", "name": "a name", "choices": ["a", "b"]},
                {"type": "string", "name": "something"},
            ],
            iscollection=False,
        )
        ...
        self._create_and_save_with_datatype(
            [
                {"type": "choice", "choices": ["a", "b"], "name": "a name"},
                {"type": "string", "name": "something"},
            ],
            iscollection=True,
        )
    ...
    def test_not_iscollection_requires_only_a_dict(self):
        ...
        self._create_and_save_with_datatype(
            {"type": "choice", "choices": ["a", "b"]}, iscollection=False
        )
    ...
    def test_subfield_cannot_be_called_id(self):
        self.assertRaises(
            ...
            self._create_and_save_with_datatype,
            [
                {"type": "choice", "name": "id", "choices": ["a", "b"]},
                {"type": "string", "name": "something"},
            ],
            iscollection=True,
        )
        ...
        self._create_and_save_with_datatype(
            [
                {"type": "choice", "name": "anything else", "choices": ["a", "b"]},
                {"type": "string", "name": "something"},
            ],
            iscollection=True,
        )
    ...
    def test_default_values(self):
        ...
        self._create_and_save_with_datatype(
            [
                ...
                {"type": "string", "name": "something", "default": "anything"},
            ],
            iscollection=True,
        )
    ...
    def test_create_multiple_choice_udf(self):
        UserDefinedFieldDefinition.objects.create(
            instance=self.instance,
            model_type="Plot",
            datatype=json.dumps({"type": "multichoice", "choices": ["a", "b"]}),
            iscollection=False,
            name="a name",
        )
...
class ScalarUDFInstanceIsolationTest(OTMTestCase):
    def setUp(self):
        ...
        for i in range(len(self.instances)):
            set_write_permissions(
                self.instances[i], self.commander_users[i], "Plot", ["udf:Test choice"]
            )
        self.choice_udfds = [
            UserDefinedFieldDefinition.objects.create(
                instance=i,
    ...
    def test_update_choice_value_in_one_instance(self):
        ...
        for i in range(len(self.plots)):
            ...
            self.assertEqual(self.plots[i].udfs["Test choice"], "a")
            self.assertEqual(audit.current_value, "a")
        ...
        self.assertEqual(self.plots[0].udfs["Test choice"], "m")
        self.assertEqual(audit0.current_value, "m")

        choice0 = UserDefinedFieldDefinition.objects.get(pk=self.choice_udfds[0].pk)

        self.assertEqual(set(choice0.datatype_dict["choices"]), {"m", "b", "c"})
        ...
        self.assertEqual(self.plots[1].udfs["Test choice"], "a")
        self.assertEqual(audit0.current_value, "a")

        choice1 = UserDefinedFieldDefinition.objects.get(pk=self.choice_udfds[1].pk)

        self.assertEqual(set(choice1.datatype_dict["choices"]), {"a", "b", "c"})
...
class ScalarUDFTest(OTMTestCase):
    ...
    def _test_datatype(self, field, value):
        ...
        self.assertEqual(self.plot.udfs[field], value)
    ...
    def test_int_validation_non_integer(self):
        self.assertRaises(ValidationError, self._test_datatype, "Test int", 42.3)

        self.assertRaises(ValidationError, self._test_datatype, "Test int", "blah")
    ...
    def test_float_validation(self):
        self.assertRaises(ValidationError, self._test_datatype, "Test float", "blah")

    def test_cant_update_choices_on_non_choice_model(self):
        floatfield = UserDefinedFieldDefinition.objects.filter(name="Test float")

        self.assertRaises(ValidationError, floatfield[0].update_choice, "a", "b")

    def test_update_invalid_choice(self):
        self.assertRaises(
            ValidationError, self.choice_udfd.update_choice, "WHAT?????", "m"
        )
    ...
    def test_multiple_invalid_choices(self):
        ...
        self.assertValidationErrorDictContainsKey(ve.exception, "udf:Test int")
        self.assertValidationErrorDictContainsKey(ve.exception, "udf:Test float")
    ...
    def test_empty_choice_deletes_field(self):
        ...
        count = Plot.objects.filter(**{"udfs__Test choice": "a"}).count()
        ...
        count = Plot.objects.filter(**{"udfs__has_key": "Test choice"}).count()
    ...
    def test_delete_choice_value(self):
        ...
        self.assertEqual(self.plot.udfs["Test choice"], "a")
        self.assertEqual(audit.current_value, "a")
        ...
        self.assertEqual(self.plot.udfs["Test choice"], None)
        self.assertEqual(audit.exists(), False)

        choice = UserDefinedFieldDefinition.objects.get(pk=self.choice_udfd.pk)

        self.assertEqual(set(choice.datatype_dict["choices"]), {"b", "c"})
    ...
    def test_delete_multichoice_value(self):
        ...
        self.assertEqual(self.plot.udfs["Test multichoice"], ["a"])
        self.assertEqual(json.loads(audit.current_value), ["a"])
        ...
        choice = UserDefinedFieldDefinition.objects.get(pk=self.multichoice_udfd.pk)
    ...
    def test_update_multichoice_value(self):
        ...
        self.assertEqual(self.plot.udfs["Test multichoice"], ["weird \\\\\\1a2chars"])
        self.assertEqual(json.loads(audit.current_value), ["weird \\\\\\1a2chars"])

        choice = UserDefinedFieldDefinition.objects.get(pk=self.multichoice_udfd.pk)

        self.assertEqual(
            set(choice.datatype_dict["choices"]), {"weird \\\\\\1a2chars", "b", "c"}
        )
        ...
        self.assertEqual(
            set(choice.datatype_dict["choices"]), {"weird \\\\\\1a2chars", "d", "c"}
        )
    ...
    def test_update_choice_value(self):
        ...
        self.assertEqual(self.plot.udfs["Test choice"], "m")
        self.assertEqual(audit.current_value, "m")

        choice = UserDefinedFieldDefinition.objects.get(pk=self.choice_udfd.pk)

        self.assertEqual(set(choice.datatype_dict["choices"]), {"m", "b", "c"})
    ...
    def test_choice_validation(self):
        self.assertRaises(
            ValidationError, self._test_datatype, "Test choice", "bad choice"
        )
    ...
    def test_returns_none_for_empty_but_valid_udfs(self):
        self.assertEqual(self.plot.udfs["Test string"], None)

    def test_raises_keyerror_for_invalid_udf(self):
        self.assertRaises(KeyError, lambda: self.plot.udfs["RaNdoName"])
...
class CollectionUDFTest(OTMTestCase):
    def setUp(self):
        ...
        self.commander_user = make_commander_user(self.instance)
        set_write_permissions(
            self.instance, self.commander_user, "Plot", ["udf:Stewardship"]
        )
    ...
    def test_can_update_choice_option(self):
        stews = [{"action": "water", "height": 42}, {"action": "prune", "height": 12}]

        self.plot.udfs["Stewardship"] = stews
        self.plot.save_with_user(self.commander_user)

        plot = Plot.objects.get(pk=self.plot.pk)
        audits = [a.current_value for a in plot.audits().filter(field="udf:action")]
    ...
    def test_cannot_save_with_invalid_field_name(self):
        self.plot.udfs["Stewardship"] = [
            {"action": "water", "height": 32, "random": "test"}
        ]
    ...
    def test_cannot_save_with_invalid_value(self):
        self.plot.udfs["Stewardship"] = [{"action": "water", "height": "too high"}]
...
class UdfDeleteTest(OTMTestCase):
    ...
    def test_delete_udf_deletes_perms_collection(self):
        ...
        qs = FieldPermission.objects.filter(
            field_name="udf:Test choice", model_name="Plot"
        )
    ...
    def test_delete_udf_deletes_perms_value(self):
        set_write_permissions(
            self.instance, self.commander_user, "Plot", ["udf:Test string"]
        )
        ...
        qs = FieldPermission.objects.filter(
            field_name="udf:Test string", model_name="Plot"
        )
    ...
    def test_delete_cudf_deletes_mobile_api_field_group(self):
        ...
        plot_udf_def = UserDefinedFieldDefinition(
            ...
            datatype=json.dumps(
                [
                    {"name": "pick", "type": "choice", "choices": ["1", "2", "3"]},
                    {"type": "int", "name": "times"},
                ]
            ),
            iscollection=True,
        ...
        updated_instance = Instance.objects.get(pk=self.instance.pk)
        self.assertEquals(
            1, len(updated_instance.mobile_api_fields[1]["collection_udf_keys"])
        )
...
class UdfCreateTest(UdfCRUTestCase):
    ...
    def test_adds_udf_to_role_when_created(self):
        ...
        for role in roles_in_instance:
            perms = [
                perm.field_name for perm in role_field_permissions(role, self.instance)
            ]
    ...
    def test_invalid_choice_list(self):
        body = {"udf.name": "cool udf", "udf.model": "Plot", "udf.type": "choice"}
    ...
    def test_missing_params(self):
        body = {"udf.model": "Plot", "udf.type": "string", "udf.choices": []}

        self.assertRaises(ValidationError, udf_create, body, self.instance)

        body = {"udf.name": "cool udf", "udf.type": "string", "udf.choices": []}

        self.assertRaises(ValidationError, udf_create, body, self.instance)

        body = {"udf.name": "cool udf", "udf.model": "Plot"}

        self.assertRaises(ValidationError, udf_create, body, self.instance)

    def test_empty_name(self):
        body = {"udf.name": "", "udf.model": "Plot", "udf.type": "string"}

        self.assertRaises(ValidationError, udf_create, body, self.instance)

    def test_duplicate_name(self):
        body = {"udf.name": "Test choice", "udf.model": "Plot", "udf.type": "string"}

        self.assertRaises(ValidationError, udf_create, body, self.instance)

    def test_invalid_model_name(self):
        body = {"udf.name": "Testing choice", "udf.model": "Shoe", "udf.type": "string"}
