...
class PermissionsTestCase(LocalMediaTestCase):
    ...
    def _make_empty_role(self, name):
        return Role.objects.create(
            name=name, instance=self.instance, rep_thresh=0, default_permission_level=0
        )

    def _add_new_permission(self, role, Model, name):
        content_type = ContentType.objects.get_for_model(Model)
        perm = Permission.objects.create(
            codename=name, name=name, content_type=content_type
        )
...
class PermsTest(PermissionsTestCase):
    ...
    def test_none_perm(self):
        self.assertEqual(
            False, perms._allows_perm(Role(), "NonExistentModel", any, "allows_reads")
        )
    ...
    def test_plot_is_deletable(self):
        ...
        self.assertTrue(
            perms.is_deletable(user_yes.get_instance_user(self.instance), plot)
        )
    ...
    def test_tree_photo_is_deletable(self):
        ...
        self._add_builtin_permission(self.role_yes, TreePhoto, "delete_treephoto")
        user_yes = make_user(
            instance=self.instance, make_role=lambda inst: self.role_yes
        )
        self.assertTrue(
...
class WritableTest(PermissionsTestCase):
    def test_plot_is_writable_if_can_create_tree(self):
        ...
        self.commander_role = self.commander_user.get_instance_user(self.instance).role
        ...
        add_tree_perm = Permission.objects.get(
            content_type=content_type, codename="add_tree"
        )
        ...
        self.assertRaises(AuthorizeException, plot2.save_with_user, self.tree_only_user)
        ...
        self.assertTrue(perms.map_feature_is_writable(self.tree_only_role, self.plot))
