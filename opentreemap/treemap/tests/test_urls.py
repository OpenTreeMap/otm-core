...
class UrlTestCase(OTMTestCase):
    def assert_status_code(self, url, code, method="GET", data="", content_type=None):
        send = {
            "GET": self.client.get,
            "PUT": self.client.put,
        ...
        self.assertEqual(
            ...
            code,
            "Actual code [%s] Expected code [%s]" % (response.status_code, code),
        )
        return response
...
class RootUrlTests(UrlTestCase):
    ...
    def test_favicon(self):
        self.assert_redirects_to_static_file("/favicon.png", "/static/img/favicon.png")
    ...
    def test_user_audits(self):
        ...
        self.assert_template(
            "/users/%s/edits/" % username, "treemap/recent_user_edits.html"
        )
        self.assert_template(
            "/users/%s/edits/?instance_id=%s" % (username, self.instance.id),
...
class TreemapUrlTests(UrlTestCase, LocalMediaTestCase):
    ...
    def test_anonymous_boundary(self):
        ...
        self.assert_200(
            url,
            method="POST",
            data=json.dumps({"polygon": [[0, 0], [1, 0], [1, 1], [0, 1], [0, 0]]}),
    ...
    def test_edits(self):
        self.assert_template(self.prefix + "edits/", "treemap/edits.html")
    ...
    def test_plot_detail_update(self):
        ...
        self.assert_200(
            self.prefix + "features/%s/" % plot.id,
            "PUT",
    ...
    def test_plot_detail_update_invalid(self):
        self.assert_401(
            self.prefix + "features/999/", "PUT", json.dumps({"plot.length": "1"})
        )
    ...
    def test_rotate_tree_photo(self):
        ...
        url = self.prefix + "features/%s/photo/%s" % (tree.plot.id, tp.id)
        self.assert_200(url, method="POST", data={"degrees": -90})
    ...
    def test_instance_settings_js(self):
        self.assert_template(self.prefix + "config/settings.js", "treemap/settings.js")

    def test_benefit_search(self):
        self.assert_template(
            self.prefix + "benefit/search", "treemap/partials/eco_benefits.html"
        )
...
class InstanceUrlTests(RequestTestCase):
    def setUp(self):
        make_instance(name="The inztance", is_public=True, url_name="ThEiNsTaNCe")
