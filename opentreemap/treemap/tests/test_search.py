...
def _setup_collection_udfs(instance, user):
    plotstew = make_collection_udf(
        instance, model="Plot", datatype=COLLECTION_UDF_DATATYPE
    )
    treestew = make_collection_udf(
        instance, model="Tree", datatype=COLLECTION_UDF_DATATYPE
    )
...
def _setup_models_for_collections(
    instance, user, point, plot_collection, tree_collection
):
    ...
    p2, __ = _create_tree_and_plot(
        ...
        point,
        plotudfs={
            plot_collection.name: [{"action": "water", "date": "2014-11-26 00:00:00"}]
        },
        treeudfs={
            tree_collection.name: [{"action": "prune", "date": "2014-06-23 00:00:00"}]
        },
    )
...
class FilterParserScalarTests(OTMTestCase):
    def setUp(self):
        self.parse_dict_value = partial(
            search._parse_dict_value_for_mapping, search.PREDICATE_TYPES
        )

    def test_key_parser_plots(self):
        # Plots searches on plot go directly to a field
        match = search._parse_predicate_key(
            "plot.width", mapping=search.DEFAULT_MAPPING
        )
        self.assertEqual(match, ("plot", "", "width"))

    def test_udf_fields_look_good(self):
        match = search._parse_predicate_key(
            "plot.udf:The 1st Planter", mapping=search.DEFAULT_MAPPING
        )
    ...
    def test_combinator_and(self):
        ...
        self.assertEqual(
            destructure_query_set(ands), destructure_query_set(qa & qb & qc)
        )
    ...
    def test_combinator_invalid_combinator(self):
        ...
        # Error if not AND,OR
        self.assertRaises(
            search.ParseException, search._apply_combinator, "ANDarg", [qa, qb]
        )

        self.assertRaises(search.ParseException, search._apply_combinator, qc, [qa, qb])

    def test_combinator_invalid_empty(self):
        # Error if empty
        self.assertRaises(search.ParseException, search._apply_combinator, "AND", [])
    ...
    def test_constraints_invalid_groups(self):
        ...
        self.assertRaises(
            search.ParseException, self.parse_dict_value, {"IS": "what", "MIN": 3}
        )

    def test_constraints_invalid_keys(self):
        self.assertRaises(
            search.ParseException, self.parse_dict_value, {"EXCLUSIVE": 9}
        )

        self.assertRaises(
            search.ParseException, self.parse_dict_value, {"IS NOT VALID KEY": "what"}
        )
    ...
    def test_contraint_max_with_exclusive(self):
        const = self.parse_dict_value({"MAX": {"VALUE": 5, "EXCLUSIVE": True}})
        self.assertEqual(const, {"__lt": 5})

        const = self.parse_dict_value({"MAX": {"VALUE": 5, "EXCLUSIVE": False}})
        self.assertEqual(const, {"__lte": 5})

    def test_constraints_min_and_max(self):
        const = self.parse_dict_value(
            {"MIN": 5, "MAX": {"VALUE": 9, "EXCLUSIVE": False}}
        )
        self.assertEqual(const, {"__lte": 9, "__gte": 5})

    def test_parse_species_predicate(self):
        pred = search._parse_scalar_predicate(
            {"species.id": 113, "species.flowering": True},
            mapping=search.DEFAULT_MAPPING,
        )

        target = (
            "AND",
            {("tree__species__id", 113), ("tree__species__flowering", True)},
        )
    ...
    def test_like_predicate(self):
        pred = search._parse_scalar_predicate(
            {"tree.steward": {"LIKE": "thisisatest"}}, mapping=search.DEFAULT_MAPPING
        )
    ...
    def test_parse_predicate(self):
        pred = search._parse_scalar_predicate(
            {
                "plot.width": {"MIN": 5, "MAX": {"VALUE": 9, "EXCLUSIVE": False}},
                "tree.height": 9,
            },
            mapping=search.DEFAULT_MAPPING,
        )

        p1 = ("AND", {("width__lte", 9), ("width__gte", 5), ("tree__height", 9)})

        self.assertEqual(destructure_query_set(pred), p1)

        pred = search._parse_scalar_predicate(
            {"tree.leaf_type": {"IS": 9}, "tree.last_updated_by": 4},
            mapping=search.DEFAULT_MAPPING,
        )
    ...
    def test_parse_filter_or(self):
        ...
        p1 = (
            "AND",
            frozenset({("width__lte", 9), ("width__gte", 5), ("tree__height", 9)}),
        )

        p2 = ("AND", frozenset({("tree__leaf_type", 9), ("tree__last_updated_by", 4)}))
...
class FilterParserCollectionTests(OTMTestCase):
    def _setup_tree_and_collection_udf(self):
        ...
        self.plotstew, self.treestew = _setup_collection_udfs(instance, commander)
    ...
    def test_key_parser_tree_collection_udf(self):
        ...
        match = search._parse_predicate_key(
            "udf:tree:52.action", mapping=search.DEFAULT_MAPPING
        )
    ...
    def test_key_parser_plot_collection_udf(self):
        # UDF searches go on the specified model's id
        match = search._parse_predicate_key(
            "udf:plot:52.action", mapping=search.DEFAULT_MAPPING
        )
    ...
    def test_parse_collection_udf_nested_pass_numeric_comparison(self):
        self._setup_tree_and_collection_udf()
        agility = make_collection_udf(
            self.instance,
            model="Tree",
            name="Agility",
            datatype=[{"type": "float", "name": "current"}],
        )
        set_write_permissions(self.instance, self.commander, "Tree", ["udf:Agility"])
    ...
    def test_parse_collection_udf_nested_pass_date_comparison(self):
        ...
        pred = search._parse_query_dict(
            {"udf:tree:%s.date" % self.treestew.pk: {"MAX": "2014-05-01 00:00:00"}},
            mapping=search.DEFAULT_MAPPING,
        )
    ...
    def test_parse_collection_udf_date_and_action_should_fail(self):
        ...
        _setup_models_for_collections(instance, commander, point, plotstew, treestew)
        ...
        pred = search._parse_query_dict(
            {
                ...
                {"MIN": "2013-09-01 00:00:00", "MAX": "2013-10-31 00:00:00"},
            },
            mapping=search.DEFAULT_MAPPING,
        )
...
class SearchTests(OTMTestCase):
    ...
    def _setup_udfs(self):
        set_write_permissions(
            self.instance, self.commander, "Plot", ["udf:Test string", "udf:Test date"]
        )
        set_write_permissions(self.instance, self.commander, "Tree", ["udf:Test float"])
        ...
        p1, __ = self.create_tree_and_plot(
            plotudfs={"Test string": "testing foo", "Test date": datetime(2010, 1, 9)},
            treeudfs={"Test float": 9.2},
        )

        p2, __ = self.create_tree_and_plot(
            plotudfs={
                "Test string": "testing baz or fish",
        ...
        p3, __ = self.create_tree_and_plot(
            plotudfs={"Test string": "baz", "Test date": datetime(2014, 1, 9)},
    ...
    def test_udf_numeric_search(self):
        ...
        self.assertEqual(
            {p1, p3},
            self._execute_and_process_filter({"tree.udf:Test float": {"MAX": 10.0}}),
        )
    ...
    def test_udf_like_search(self):
        ...
        self.assertEqual(
            {p2, p3},
            self._execute_and_process_filter({"plot.udf:Test string": {"LIKE": "baz"}}),
        )
    ...
    def test_udf_direct_search(self):
        ...
        self.assertEqual(
            {p3},
            self._execute_and_process_filter({"plot.udf:Test string": {"IS": "baz"}}),
        )
    ...
    def test_boundary_search(self):
        ...
        boundary1_filter = json.dumps({"plot.geom": {"IN_BOUNDARY": b1.pk}})

        plots = search.Filter(boundary1_filter, "", self.instance).get_objects(Plot)

        self.assertEqual({plot1.pk}, {p.pk for p in plots})

        boundary2_filter = json.dumps({"plot.geom": {"IN_BOUNDARY": b2.pk}})

        plots = search.Filter(boundary2_filter, "", self.instance).get_objects(Plot)

        self.assertEqual({plot1.pk, plot2.pk}, {p.pk for p in plots})

        boundary3_filter = json.dumps({"plot.geom": {"IN_BOUNDARY": b3.pk}})

        plots = search.Filter(boundary3_filter, "", self.instance).get_objects(Plot)
    ...
    def test_diameter_units(self):
        ...
        plots = search.Filter(filter, "", self.instance).get_objects(Plot)
    ...
    def test_diameter_min_filter(self):
        ...
        diameter_range_filter = json.dumps({"tree.diameter": {"MIN": 3.0}})

        plots = search.Filter(diameter_range_filter, "", self.instance).get_objects(
            Plot
        )
    ...
    def test_diameter_max_filter(self):
        ...
        diameter_range_filter = json.dumps({"tree.diameter": {"MAX": 3.0}})
    ...
    def test_diameter_range_filter(self):
        ...
        diameter_range_filter = json.dumps({"tree.diameter": {"MAX": 7.0, "MIN": 3.0}})
    ...
    def test_like_filter(self):
        ...
        species_like_filter = json.dumps({"species.common_name": {"LIKE": "s a tes"}})

        plots = search.Filter(species_like_filter, "", self.instance).get_objects(Plot)
    ...
    def test_display_filter_filters_out_models(self):
        ...
        plots = search.Filter("", '["FireHydrant"]', self.instance).get_objects(Plot)
    ...
    def test_empty_plot_filter(self):
        ...
        plots = search.Filter("", '["EmptyPlot"]', self.instance).get_objects(Plot)
    ...
    def test_tree_filter(self):
        ...
        plots = search.Filter("", '["Tree"]', self.instance).get_objects(Plot)
    ...
    def test_plot_filter(self):
        ...
        plots = search.Filter("", '["Plot"]', self.instance).get_objects(Plot)
    ...
    def test_cudf_range_search(self):
        ...
        self.assertEqual(
            {p3},
            self._execute_and_process_filter(
                {"udf:plot:%s.date" % self.plotstew.pk: {"MIN": "2015-01-01 00:00:00"}}
            ),
        )
            ...
            self._execute_and_process_filter(
                {
                    "udf:plot:%s.action" % self.plotstew.pk: {"IS": "prune"},
                    "udf:plot:%s.date"
                    % self.plotstew.pk: {"MAX": "2014-05-01 00:00:00"},
                }
            ),
        )
            ...
            self._execute_and_process_filter(
                {
                    "udf:plot:%s.action" % self.plotstew.pk: {"IS": "prune"},
                    "udf:plot:%s.date"
                    % self.plotstew.pk: {"MIN": "2013-09-15 00:00:00"},
                }
            ),
        )
