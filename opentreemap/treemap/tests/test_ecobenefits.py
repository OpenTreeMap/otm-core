...
from treemap.ecocache import (
    get_cached_benefits,
    get_cached_plot_count,
...
class EcoTest(EcoTestCase):
    ...
    def test_eco_benefit_sanity(self):
        rslt, basis, error = TreeBenefitsCalculator().benefits_for_object(
            self.instance, self.tree.plot
        )
        ...
        self.assert_benefit_value(bens, BenefitCategory.STORMWATER, "gal", 3185)
        self.assert_benefit_value(bens, BenefitCategory.CO2, "lbs", 563)
        self.assert_benefit_value(bens, BenefitCategory.CO2STORAGE, "lbs", 6575)

    def testSearchBenefits(self):
        request = make_request(
            {"q": json.dumps({"tree.readonly": {"IS": False}})}
        )  # all trees
    ...
    def test_group_basis_empty(self):
        basis = {}
        example = {
            "group1": {"n_objects_used": 5, "n_objects_discarded": 8},
            "group2": {"n_objects_used": 10, "n_objects_discarded": 12},
        }
    ...
    def test_group_basis_combine_new_group(self):
        ...
        new_group = {"group2": {"n_objects_used": 13, "n_objects_discarded": 4}}
        target = {
            "group1": {"n_objects_used": 5, "n_objects_discarded": 8},
            "group2": {"n_objects_used": 13, "n_objects_discarded": 4},
        }
    ...
    def test_group_basis_combine_existing_groups(self):
        basis = {"group1": {"n_objects_used": 5, "n_objects_discarded": 8}}
        update_group = {"group1": {"n_objects_used": 13, "n_objects_discarded": 4}}
    ...
    def test_combine_benefit_groups_no_overlap(self):
        ...
        target = {"group1": base_group["group1"], "group2": new_group["group2"]}
    ...
    def test_annotates_basis(self):
        basis = {
            "group1": {"n_objects_used": 5, "n_objects_discarded": 15},
            "group2": {"n_objects_used": 2, "n_objects_discarded": 18},
        }
        target = {
            "group1": {
                "n_objects_used": 5,
