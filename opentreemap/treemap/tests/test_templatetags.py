...
class UserCanReadTagTest(OTMTestCase):
    ...
    def _render_basic_template_with_vars(self, user, plot):
        return UserCanReadTagTest.basic_template.render(
            Context({"request": {"user": user}, "plot": plot})
        ).strip()
    ...
    def test_works_with_empty_user_with_perm(self):
        ...
        for plevel in perms:
            ...
            self.assertEqual(
                self._render_basic_template_with_vars(None, self.plot), "plot width 9"
            )
...
class UserCanCreateTagTest(OTMTestCase):
    ...
    def test_works_with_user_with_no_create_perms(self):
        ...
        self.assertEqual(self._render_basic_template_with_vars(user, self.plot), "")

    def test_works_with_user_with_create_perms(self):
        user = make_conjurer_user(self.instance)
        self.assertEqual(self._render_basic_template_with_vars(user, self.plot), "true")
...
class UserContentTagTests(OTMTestCase):
    ...
    def _render_template_with_user(self, template, req_user, content_user):
        return template.render(
            Context({"request": {"user": req_user}, "user": content_user})
        ).strip()
    ...
    def render_user_template(self, req_user, content_user):
        ...
        return self._render_template_with_user(user_template, req_user, content_user)
    ...
    def render_user_id_template(self, req_user, content_user):
        ...
        return self._render_template_with_user(user_id_template, req_user, content_user)
    ...
    def test_username_in_tag_hides_content_from_others(self):
        self.assertEqual(self.render_username_template(self.public_user, self.user), "")

    def test_user_in_tag_shows_content_to_user(self):
        self.assertEqual(self.render_user_template(self.user, self.user), "SECRETS!")

    def test_user_in_tag_hides_content_from_others(self):
        self.assertEqual(self.render_user_template(self.public_user, self.user), "")

    def test_user_id_in_tag_shows_content_to_user(self):
        self.assertEqual(self.render_user_id_template(self.user, self.user), "SECRETS!")

    def test_user_id_in_tag_hides_content_from_others(self):
        self.assertEqual(self.render_user_id_template(self.public_user, self.user), "")
    ...
    def test_literal_username_in_tag_hides_content_from_others(self):
        content = self.render_literal_username_template(self.public_user, self.user)
    ...
    def test_literal_user_id_in_tag_hides_content_from_others(self):
        content = self.render_literal_user_id_template(self.public_user, self.user)
        self.assertEqual(content, "")
...
class LoginForwardingTests(OTMTestCase):
    def setUp(self):
        self.instance = make_instance()
        self.literal_template = Template("{% load auth_extras %}{% login_forward %}")

    def render_template(self, template, path, instance=None):
        return template.render(
            Context({"request": make_request(path=path, instance=instance)})
        )

    def test_request_has_instance(self):
        path = "/{}/anything/goes/".format(self.instance.url_name)
        self.assertEqual(
            self.render_template(self.literal_template, path, self.instance),
            "?next=%2F{}%2Fanything%2Fgoes%2F".format(self.instance.url_name),
        )
    ...
    def test_request_path_redirect(self):
        ...
        path = "/create/"
        self.assertEqual(
            self.render_template(self.literal_template, path), "?next=%2Fcreate%2F"
        )

    def test_default_redirect(self):
        path = "/anything/else/"
        self.assertEqual(self.render_template(self.literal_template, path), "")


_TEMPLATES = deepcopy(settings.TEMPLATES)
_TEMPLATES[0]["OPTIONS"]["loaders"] = ["django.template.loaders.filesystem.Loader"]
...
class InlineFieldTagTests(OTMTestCase):
    ...
    def assert_plot_length_context_value(self, user, name, value, template_fn=None):
        if template_fn is None:
            template_fn = (
                self._form_template_with_request_user_for
                if user
        ...
        with self.settings(TEMPLATES=_TEMPLATES):
            content = template.render(
                Context(
                    {"request": {"user": user, "instance": self.instance}, "plot": plot}
                )
            ).strip()
    ...
    def test_labelless_sets_is_visible_to_true_if_user_is_not_specified(self):
        ...
        self.assert_plot_length_context_value(
            None, "field.is_visible", "True", self._form_template_labelless_for
        )
    ...
    def test_udf_sets_is_visible_to_true_for_user_with_perms(self):
        user = make_user(
            self.instance, username="udf_user", make_role=lambda _: self.udf_role
        )
    ...
    def test_create_uses_new_model(self):
        ...
        with self.settings(TEMPLATES=_TEMPLATES):
            content = template.render(
                Context({"request": {"user": self.observer, "instance": self.instance}})
            ).strip()
...
class PartialTagTest(OTMTestCase):
    def _assert_renders_as(self, template_text, subdict_name, expected):
        context = Context({"mine": {"val": "yes"}, "yours": {"val": "no"}})
...
class AddParamsTest(OTMTestCase):
    def test_add_first_param(self):
        self.assertEqual(add_params("/a/b#hash", foo=1), "/a/b?foo=1#hash")

    def test_add_params_to_existing_param(self):
        self.assertEqual(
            add_params("/a/b?baz=0#hash", foo=1, bar=2), "/a/b?bar=2&foo=1&baz=0#hash"
        )
...
class InThreadOrderTest(OTMTestCase):
    def test_in_thread_order(self):
        ...
        self.assertEqual(
            [c.id for c in [root1, nested1, nested12, nested11, root2]],
