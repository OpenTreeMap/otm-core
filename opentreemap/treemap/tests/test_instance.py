...
from treemap.tests import make_instance, make_simple_boundary, make_anonymous_boundary
...
class InstanceMobileApiFieldsTests(OTMTestCase):
    def setUp(self):
        ...
        make_collection_udf(
            self.instance, model="Plot", name="Caring", datatype=datatype
        )
        make_collection_udf(
            self.instance, model="Tree", name="Caring", datatype=datatype
        )
    ...
    def assert_raises_code(self, msg, fields):
        ...
        messages = {force_text(e) for e in val_err.message_dict["mobile_api_fields"]}
    ...
    def test_basic_errors(self):
        ...
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_no_header"],
        ...
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_no_keys"], [{"header": "Trees"}]
        )
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_no_keys"],
            [{"header": "Trees", "collection_udf_keys": None}],
        )

        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_both_keys"],
            [
                {
                    "header": "Trees",
    ...
    def test_collection_udf_errors(self):
        ...
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_no_sort_key"],
            [{"header": "Trees", "collection_udf_keys": ["plot.udf:Stewardship"]}],
        )

        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_missing_cudf"],
            [{"header": "Trees", "collection_udf_keys": ["plot.udf:Not a field"]}],
        )
        # UDF: Name exists, but it is not a collection field
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_missing_cudf"],
            [{"header": "Trees", "collection_udf_keys": ["plot.udf:Name"]}],
        )
        # Similar for plot.width
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_has_missing_cudf"],
            [{"header": "Trees", "collection_udf_keys": ["plot.width"]}],
        )
    ...
    def test_standard_errors(self):
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["duplicate_fields"],
            [
                {
                    "header": "Best Fields",
                    "model": "tree",
        ...
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["group_missing_model"],
            [{"header": "Stuff", "model": "hydrant", "field_keys": ["hydrant.valves"]}],
        )
        ...
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["missing_field"],
            [
                {
                    "header": "Trees",
                    "model": "tree",
        ...
        self.assert_raises_code(
            INSTANCE_FIELD_ERRORS["missing_field"],
            [{"header": "Trees", "model": "plot", "field_keys": ["plot.doesnotexist"]}],
        )
...
class InstanceWebDetailFieldsTests(OTMTestCase):
    ...
    def assert_raises_code(self, msg, fields):
        ...
        messages = {force_text(e) for e in val_err.message_dict["web_detail_fields"]}
        self.assertIn(force_text(msg), messages)
