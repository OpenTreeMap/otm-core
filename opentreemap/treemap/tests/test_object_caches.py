...
from treemap.lib.object_caches import (
    clear_caches,
...
from treemap.tests import make_instance, make_commander_user, make_user
...
class PermissionsCacheTest(TestCase):
    ...
    def get_single_perm_qs(self, role, model_name="Plot", field_name="owner_orig_id"):
    ...
    def delete_permission(self, role, model_name="Plot", field_name="owner_orig_id"):
        qs = self.get_single_perm_qs(role, model_name, field_name)
        qs.delete()
...
class UDFDefinitionCacheTest(TestCase):
    ...
    def test_delete(self):
        self.assert_udf_name("Tree", "a")  # load cache
        UserDefinedFieldDefinition.objects.filter(model_type="Tree").delete()
    ...
    def test_external_update(self):
        ...
        # invalidating the cache, then updating the instance timestamp.
        # update() sends no signals, so cache won't be invalidated.
        UserDefinedFieldDefinition.objects.filter(model_type="Tree").update(name="c")
