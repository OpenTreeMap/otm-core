...
class InstanceAdvancedSearch(OTMTestCase):
    ...
    def test_missing_filters(self):
        ...
        self.instance.search_config = {"missing": [{"identifier": "tree.id"}]}
        self.assert_search_present(
            missing={
    ...
    def test_section_empty_when_field_not_visible(self):
        perm = FieldPermission.objects.filter(
            role__name="commander", model_name="Tree", field_name="diameter"
        )
...
class InstanceMobileSearch(OTMTestCase):
    ...
    def test_missing_filters(self):
        ...
        self.assert_search_present(
            missing={"label": "Show Missing Trees", "identifier": "tree.id",}
        )
