...
class HashModelTest(OTMTestCase):
    ...
    def test_changing_fields_changes_hash(self):
        ...
        h1 = plot.hash
        tree = Tree(plot=plot, instance=self.instance, readonly=False)
...
class ModelUnicodeTests(OTMTestCase):
    def setUp(self):
        ...
        self.plot = Plot(
            geom=Point(1, 1), instance=self.instance, address_street="123 Main Street"
        )
        ...
        self.reputation_metric = ReputationMetric(
            instance=self.instance, model_name="Tree", action="Test Action"
        )
    ...
    def test_audit_model(self):
        self.assertEqual(
            unicode(self.audit),
            "pk=%s - action=Update - Tree.readonly:(1) - True => False" % self.audit.pk,
        )

    def test_reputation_metric_model(self):
        self.assertEqual(
            unicode(self.reputation_metric), "Test Instance - Tree - Test Action"
        )
...
class TreeTest(OTMTestCase):
    ...
    def test_negative_canopy_height_fails_validation(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            self.tree.save_with_user(self.user)
        self.assertValidationErrorDictContainsKey(cm.exception, "canopy_height")
...
class InstanceTest(OTMTestCase):
    ...
    def test_can_read_and_write_config(self):
        ...
        self.assertEqual(
            reloaded_instance.config,
            {"config_value": "test", "hold up": {"nested": "true"}},
        )
...
class ValidationMixinTest(SimpleTestCase):
    ...
    def test_max_value(self):
        with self.assertRaises(ValidationError):
            Car(5).validate_positive_nullable_float_field("weight", max_value=2)
