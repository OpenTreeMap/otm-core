...
from treemap.routes import root_settings_js, instance_settings_js, instance_user_page
...
from treemap.views.misc import (
...
from treemap.views.user import (
    user_audits,
    upload_user_photo,
    update_user,
    forgot_username,
...
class StaticPageViewTest(ViewTestCase):
    def setUp(self):
        ...
        self.staticPage = StaticPage(
            content="content", name="faq", instance=self.instance
        )
    ...
    def test_instance_mismatch(self):
        self.assertRaises(Http404, static_page, None, self.otherInstance, "blah")

    def test_missing_name(self):
        self.assertRaises(Http404, static_page, None, self.instance, "missing")
...
class BoundaryViewTest(ViewTestCase):
    ...
    def test_boundary_to_geojson_view(self):
        ...
        self.instance.boundaries.add(boundary)
        self.instance.save()
        response = boundary_to_geojson(make_request(), self.instance, boundary.pk)

        self.assertEqual(
            response.content, boundary.geom.transform(4326, clone=True).geojson
        )
    ...
    def test_add_anonymous_boundary_view(self):
        ...
        content = add_anonymous_boundary(make_request(body=json.dumps(request_dict)))

        self.assertIn("id", content)
        boundary_id = content["id"]
        anonymous_boundary = Boundary.all_objects.get(pk=boundary_id)

        gjs_response = boundary_to_geojson(make_request(), self.instance, boundary_id)

        self.assertEqual(
            gjs_response.content,
            anonymous_boundary.geom.transform(4326, clone=True).geojson,
        )
    ...
    def test_autocomplete_view_limit(self):
        response = boundary_autocomplete(make_request({"max_items": 2}), self.instance)
    ...
    def _assert_response_is_srid_3857_distance(self, response, distance):
        ...
        response_upper_left = Point(json_response["coordinates"][0][0][0], srid=4326)
        self.assertAlmostEqual(response_upper_left.get_x(), upper_left_4326.get_x())
...
class DeleteOwnPhotoTest(TreePhotoTestCase):
    ...
    def _delete_photo(self, user):
        ...
        return delete_photo(
            make_request(method="DELETE", user=user, instance=self.instance),
            self.instance,
            self.plot.pk,
            old_photo.pk,
        )
    ...
    def test_delete_own_photo_after_role_demotion(self):
        admin = make_user(
            self.instance, username="headhoncho", make_role=self.user.get_role
        )
...
class TreePhotoRotationTest(TreePhotoTestCase):
    ...
    def test_tree_photo_rotation(self):
        ...
        self.assertAlmostEqual(
            old_photo.image.width, rotated_photo.image.height, delta=1
        )
        self.assertAlmostEqual(
            old_photo.image.height, rotated_photo.image.width, delta=1
        )

        self.assertAlmostEqual(
            old_photo.thumbnail.width, rotated_photo.thumbnail.height, delta=1
        ),
        self.assertAlmostEqual(
            old_photo.thumbnail.height, rotated_photo.thumbnail.width, delta=1
        )
...
class ApproveOrRejectPhotoTest(TreePhotoTestCase):
    ...
    def test_approve_photo_no_pending(self):
        ...
        approve_or_reject_photos(
            make_request({"ids": str(tp.pk)}, user=self.user), self.instance, "approve"
        )
    ...
    def test_approve_multiple_photos(self):
        ...
        all_audits = list(
            audit for tp in TreePhoto.objects.all() for audit in tp.audits()
        )

        approve_or_reject_photos(
            make_request({"ids": tp_ids}, user=self.user), self.instance, "approve"
        )
    ...
    def test_reject_photo_no_pending(self):
        ...
        approve_or_reject_photos(
            make_request({"ids": str(tp.pk)}, user=self.user), self.instance, "reject"
        )
...
class PlotImageUpdateTest(LocalMediaTestCase):
    def setUp(self):
        ...
        # correctness
        self.plot = Plot(geom=self.instance.center, instance=self.instance, pk=449293)
    ...
    def _make_tree_photo_request(self, file, plot_id, tree_id=None):
        return add_tree_photo_helper(
            make_request(user=self.user, file=file), self.instance, plot_id, tree_id
        )
    ...
    def test_add_photo_to_tree(self):
        ...
        self._make_tree_photo_request(tree_image, self.plot.pk, self.tree.pk)
    ...
    def test_invalid_ids(self):
        ...
        self.assertRaises(Http404, self._make_tree_photo_request, tree_image, -1, None)
        ...
        self.assertRaises(
            Http404, self._make_tree_photo_request, tree_image, self.plot.pk, -1
        )
    ...
    def test_assigns_to_tree_if_exists(self):
        ...
        self._make_tree_photo_request(tree_image, self.plot.pk, None)
...
class UserPhotoUpdateTest(LocalMediaTestCase):
    ...
    def upload_photo(self, filename):
        ...
        return upload_user_photo(make_request(user=self.user, file=file), self.user)
...
class PlotUpdateTest(OTMTestCase):
    def setUp(self):
        ...
        self.user = make_commander_user(self.instance)
        set_write_permissions(
            self.instance, self.user, "Plot", ["udf:Test choice", "udf:Test col"]
        )
        set_write_permissions(
            self.instance, self.user, "Tree", ["udf:Test choice", "udf:Test col"]
        )
        ...
        self.tree_choice_field = UserDefinedFieldDefinition.objects.create(
            instance=self.instance,
            model_type="Tree",
            datatype=json.dumps({"type": "choice", "choices": ["foo", "bar", "baz"]}),
            iscollection=False,
            name="Test choice",
        )
        ...
        self.col_field_plot = UserDefinedFieldDefinition.objects.create(
            ...
            datatype=json.dumps(
                [
                    {"name": "pick", "type": "choice", "choices": ["a", "b", "c"]},
                    {"name": "num", "type": "int"},
                ]
            ),
            iscollection=True,
    ...
    def test_creates_new_plot(self):
        ...
        update = {"plot.geom": {"x": 4, "y": 9}, "plot.readonly": False}
    ...
    def test_does_create_tree_when_one_tree_field_is_non_empty(self):
        ...
        created_plot, updated_tree = update_map_feature(update, self.user, plot)
    ...
    def test_does_clear_field_if_tree_already_exists(self):
        ...
        updated_plot, created_tree = update_map_feature(update, self.user, self.plot)
    ...
    def test_invalid_udf_name_fails(self):
        ...
        self.assertRaises(KeyError, update_map_feature, update, self.user, self.plot)

    def test_collection_udf_works(self):
        plot_data = [{"pick": "a", "num": 4}, {"pick": "b", "num": 9}]
        tree_data = [{"pick": "c", "num": 1}, {"pick": "a", "num": 33}]

        update = {"plot.udf:Test col": plot_data, "tree.udf:Test col": tree_data}
    ...
    def test_collection_udf_errors_show_up_as_validations(self):
        ...
        self.assertRaises(
            ValidationError, update_map_feature, update, self.user, self.plot
        )
    ...
    def test_grouping_failed_udf(self):
        update = {"plot.udf:Test choice": "z", "plot.length": "bad"}
    ...
    def test_simple_update(self):
        ...
        update = {"plot.length": 20, "plot.width": 25, "plot.udf:Test choice": "b"}
    ...
    def test_edit_tree_creates_tree(self):
        ...
        self.assertIsNotNone(Plot.objects.get(pk=self.plot.pk).current_tree())
    ...
    def test_doesnt_edit_tree_if_plot_fails(self):
        ...
        update = {"plot.length": "length", "tree.height": 42}
    ...
    def test_doesnt_edit_plot_if_tree_fails(self):
        ...
        update = {"plot.length": 92, "tree.height": "height"}
...
class PlotViewTestCase(ViewTestCase):
    ...
    def get_plot_context(self, plot):
        context = plot_detail(
            make_request(user=self.user, instance=self.instance), self.instance, plot.pk
        )
        return context
...
class PlotViewTest(PlotViewTestCase):
    ...
    def test_plot_with_tree(self):
        ...
        tree = Tree(
            plot=plot_w_tree, instance=self.instance, diameter=10, species=species
        )
        tree.save_with_user(self.user)

        request = make_request(user=self.user, instance=self.instance)
        request.instance_supports_ecobenefits = self.instance.has_itree_region()
    ...
    def test_system_user_hidden_from_audit_history(self):
        ...
        self.assertEqual(
            1,
            len(Audit.objects.filter(model_id=plot.pk, model="Plot", user=system_user)),
        )
...
class PlotViewProgressTest(PlotViewTestCase):
    ...
    def test_progress_messages_decrease_when_plot_has_tree(self):
        ...
        self.assertTrue(
            len(wo_tree_context["progress_messages"])
            > len(w_tree_context["progress_messages"])
        )
        # Adding a tree without and details does not add progress
        self.assertTrue(
            wo_tree_context["progress_percent"] == w_tree_context["progress_percent"]
        )
    ...
    def test_progress_increases_when_diameter_is_added(self):
        ...
        self.assertTrue(len(context["progress_messages"]) < self.initial_message_count)

    def test_progress_increases_when_species_is_added(self):
        species = Species(common_name="test", otm_code="TEST", instance=self.instance)
        species.save_with_user(self.user)
        tree = self.plot_w_tree.current_tree()
...
class PlotViewPhotoProgressTest(TreePhotoTestCase):
    def setUp(self):
        ...
        context = plot_detail(
            make_request(user=self.user, instance=self.instance),
            self.instance,
            self.plot.pk,
        )
...
class PlotExternalLinkTest(OTMTestCase):
    ...
    def _add_instance_permission(self, role):
        ...
        perm = Permission.objects.get(
            content_type=content_type, codename=PERMISSION_VIEW_EXTERNAL_LINK
        )
    ...
    def setUp(self):
        ...
        set_attr_on_json_field(self.instance, "config.externalLink.text", "some text")
        self.instance.save()

        self.creator = make_commander_user(self.instance, username="creator")
        self.commander = make_commander_user(self.instance)

        self.empty_plot = Plot(instance=self.instance, geom=self.move(self.p1, 0.5, 0))
        self.empty_plot.save_with_user(self.creator)
        self.planted_plot = Plot(
            instance=self.instance, geom=self.move(self.p1, 0, 0.5)
        )
    ...
    def test_validate_token_template(self):
        ...
        self.assertFalse(validate_token_template("#{tree.id} #{}"))
        self.assertFalse(validate_token_template("#{tree.id} #{ tree.id }"))
        self.assertFalse(validate_token_template("#{tree.id} #{bioswale.id}"))

    def test_tree_external_link_shows(self):
        set_attr_on_json_field(
            self.instance, "config.externalLink.url", "something#{tree.id}/#hash"
        )
        self.instance.save()
        self._add_instance_permission(self.commander.get_role(self.instance))

        context = plot_detail(
            make_request(user=self.commander, instance=self.instance),
            self.instance,
            self.planted_plot.pk,
        )

        self.assertEqual(
            context["external_link"], "something{}/#hash".format(str(self.tree.pk))
        )
    ...
    def test_plot_external_link_custom_id_shows(self):
        ...
        self.assertEqual(
            context["external_link"],
            "something{}/#hash".format(str(self.empty_plot.owner_orig_id)),
        )

    def test_no_tree_no_external_link(self):
        set_attr_on_json_field(
            self.instance, "config.externalLink.url", "something#{tree.id}/more"
        )
        self.instance.save()
        self._add_instance_permission(self.commander.get_role(self.instance))
...
class RecentEditsViewTest(ViewTestCase):
    ...
    def check_audits(self, url, dicts, user=None):
        ...
        resulting_audits = [audit for audit in edits(req, self.instance)["audits"]]
    ...
    def check_user_audits(self, url, username, dicts):
        ...
        resulting_audits = [
            audit.dict() for audit in user_audits(req, username)["audits"]
        ]
    ...
    def test_multiple_deltas(self):
        ...
        self.check_user_audits(
            "/blah/?page_size=2&instance_id=%s" % self.instance.pk,
    ...
    def test_paging(self):
        ...
        self._assert_dicts_equal(
            [self.next_plot_delta], [a.dict() for a in page_one_result["audits"]]
        )
        ...
        self._assert_dicts_equal(
            [self.plot_delta], [a.dict() for a in page_two_result["audits"]]
        )
        ...
        self.assertEqual(page_two_result["audits"], page_two_again_result["audits"])

    def test_model_filtering_errors(self):
        self.assertRaises(
            Exception,
            self.check_audits,
            "/blah/?model_id=%s&page_size=1" % self.tree.pk,
            [],
        )
        ...
        self.assertRaises(
            Exception,
            self.check_audits,
            "/blah/?model_id=%s&" "models=Tree&models=Plot&page_size=1" % self.tree.pk,
            [],
        )

        self.assertRaises(
            Exception, self.check_audits, "/blah/?models=User&page_size=1", []
        )
        ...
        self.assertRaises(
            ...
            "&page_size=1&instance_id=%s" % (self.tree.pk, self.instance.pk),
            self.commander.username,
            [],
        )
        ...
        self.assertRaises(
            Exception,
            self.check_user_audits,
            "/blah/?models=User&page_size=1&instance_id=%s" % self.instance.pk,
            self.commander.username,
            [],
        )
    ...
    def test_model_filtering(self):
        ...
        generic_tree_delta = {"model": "Tree"}

        generic_plot_delta = {"model": "Plot"}
        ...
        self.check_audits("/blah/?models=Plot&page_size=5", [generic_plot_delta] * 5)

        self.check_audits("/blah/?models=Tree&page_size=5", [generic_tree_delta] * 5)
    ...
    def test_model_user_filtering(self):
        ...
        self.check_user_audits(
            "/blah/?models=Plot&page_size=3&instance_id=%s" % self.instance.pk,
            self.commander.username,
            [generic_plot_delta] * 3,
        )

        self.check_user_audits(
            "/blah/?models=Tree&page_size=3",
            self.officer.username,
    ...
    def test_user_filtering(self):
        ...
        generic_officer_delta = {"user_id": self.officer.pk}

        generic_commander_delta = {"user_id": self.commander.pk}

        self.check_audits(
            "/blah/?user=%s&page_size=3" % self.officer.pk, [generic_officer_delta] * 3
        )
    ...
    def test_pending_filtering(self):
        ...
        a = approve_or_reject_audit_and_apply(
            Audit.objects.all().order_by("-created")[0], self.commander, approved=True
        )
    ...
    def test_udf_collection_audits_appear(self):
        ...
        set_write_permissions(self.instance, self.commander, "Plot", ["udf:Stew"])
    ...
    def test_udf_collection_audits_not_shown_with_no_permissions(self):
        ...
        set_write_permissions(self.instance, self.commander, "Plot", ["udf:Stew"])
        set_read_permissions(
            self.instance, self.officer, "Plot", self.plot.tracked_fields
        )
    ...
    def test_only_show_audits_for_permitted_fields(self):
        set_invisible_permissions(
            self.instance, self.commander, "Plot", self.plot.tracked_fields
        )
        set_invisible_permissions(
            self.instance, self.commander, "Tree", self.tree.tracked_fields
        )

        req = self.factory.get("/blah/")
        req.user = self.commander
        result = edits(req, self.instance)["audits"]
        tree_audit_fields = {audit.field for audit in result if audit.model == "tree"}
        plot_audit_fields = {audit.field for audit in result if audit.model == "plot"}

        self.assertLessEqual(
            tree_audit_fields, self.tree.visible_fields(self.commander)
        )
        self.assertLessEqual(
            plot_audit_fields, self.plot.visible_fields(self.commander)
        )
        ...
        self.assertEqual(
            ...
            len(
                Audit.objects.filter(
                    model_id=self.plot.pk, model="Plot", user=system_user
                )
            ),
        )
...
class SpeciesViewTests(ViewTestCase):
    def setUp(self):
        ...
        self.species_dict = [
            ...
            {"common_name": "cherrytree", "genus": "cherritius", "cultivar": "asian"},
            {"common_name": "elm", "genus": "elmitius"},
            {"common_name": "oak", "genus": "acorn", "species": "oakenitus"},
        ]
        self.species_json = [
    ...
    def test_get_species_list(self):
        self.assertEquals(
            species_list(make_request(), self.instance), self.species_json
        )
...
class UserViewTests(ViewTestCase):
    ...
    def test_get_with_invalid_username_returns_404(self):
        self.assertRaises(Http404, user, make_request(), "no_way_this_is_a_username")
...
class UserUpdateViewTests(ViewTestCase):
    ...
    def test_empty_update_returns_ok(self):
        self.assertOk(update_user(make_request(user=self.joe), self.joe))

    def test_change_first_name(self):
        self.joe.first_name = "Joe"
        self.joe.save()
        update = b'{"user.first_name": "Joseph"}'
        self.assertOk(update_user(make_request(user=self.joe, body=update), self.joe))
        self.assertEquals(
            "Joseph",
    ...
    def test_cant_change_password_through_update_view(self):
        ...
        self.assertBadRequest(
            return_400_if_validation_errors(update_user)(
                make_request(user=self.joe, body=update), self.joe
            )
        )
...
class InstanceUserViewTests(ViewTestCase):
    ...
    def test_get_by_username_redirects(self):
        res = instance_user_page(
            make_request(), self.instance.url_name, self.commander.username
        )
        ...
        self.assertEquals(
            expected_url,
            res["Location"],
            "the view should redirect to %s not %s " % (expected_url, res["Location"]),
        )

    def test_get_with_invalid_username_redirects(self):
        test_username = "no_way_username"
        res = instance_user_page(make_request(), self.instance.url_name, test_username)
        expected_url = "/users/%s/?instance_id=%d" % (test_username, self.instance.id)
        self.assertEquals(
            res.status_code,
            302,
            "should be a 302 Found \
...
class SettingsJsViewTests(ViewTestCase):
    def assertInResponse(self, text, res):
        self.assertIn(
            text, str(res), "expected %s to be in the response:\n%s" % (text, str(res))
        )
    ...
    def test_none_tile_hosts_omits_tilehosts_setting(self):
        self.assertNotInResponse("otm.settings.tileHosts", self.get_response())

    @override_settings(TILE_HOST="{s}.a")
    def test_single_tile_host_in_tilehosts_setting(self):
        self.assertInResponse('otm.settings.tileHost = "{s}.a";', self.get_response())
...
class ScssCompilationTests(ViewTestCase):
    def test_css_content_differs_by_argument(self):
        request1 = self.factory.get(
            "", {"primary-color": "fff", "secondary-color": "fff"}
        )
        request2 = self.factory.get(
            "", {"primary-color": "000000", "secondary-color": "000"}
        )
...
class DeleteViewTests(ViewTestCase):
    ...
    def test_delete_plot_view_failure(self):
        ...
        Tree(plot=plot, instance=self.instance, diameter=10).save_with_user(self.user)
...
class ForgotUsernameTests(ViewTestCase):
    ...
    def test_sends_email_for_existing_user(self):
        resp = forgot_username(make_request({"email": self.user.email}, method="POST"))
    ...
    def test_no_email_if_doesnt_exist(self):
        resp = forgot_username(
            make_request({"email": "doesnt@exist.co.uk"}, method="POST")
        )
...
class UserAutocompleteTest(OTMTestCase):
    ...
    def test_filtering_and_sorting(self):
        self.assert_users_in_list(self.i1, {}, self.also_mike, self.maria, self.mike)
        self.assert_users_in_list(
            self.i1, {"q": "M"}, self.mike, self.maria, self.also_mike
        )
        self.assert_users_in_list(self.i1, {"q": "Mi"}, self.mike, self.also_mike)
