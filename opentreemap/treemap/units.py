...
class Convertible(object):
    ...
    def terminology(cls, instance=None):
        ...
        if instance:
            terms.update(instance.config.get("terms", {}).get(cls.__name__, {}))
        return terms
...
_unit_conversions["ft"] = {u: v * 12 for (u, v) in _unit_conversions["in"].iteritems()}
...
def get_value_display_attr(instance, category_name, value_name, key):
    if not instance:
        raise Exception(
            "Need an instance to format value %s.%s" % (category_name, value_name)
        )
    ...
    value = get_attr_from_json_field(instance, field_name) or _get_display_default(
...
def get_units(instance, category_name, value_name):
    __, units = get_value_display_attr(instance, category_name, value_name, "units")
    return units


def get_digits(instance, category_name, value_name):
    __, digits = get_value_display_attr(instance, category_name, value_name, "digits")
    return digits
...
def _is_configured_for(keys, category_name, value_name):
    defaults = settings.DISPLAY_DEFAULTS
    return (
        category_name in defaults
        and value_name in defaults[category_name]
        and keys & defaults[category_name][value_name].viewkeys()
    )


is_convertible_or_formattable = partial(_is_configured_for, {"units", "digits"})
...
def storage_to_instance_units_factor(instance, category_name, value_name):
    ...
    if instance_unit not in conversion_dict.keys():
        raise Exception(
            "Cannot convert from [%s] to [%s]" % (storage_unit, instance_unit)
        )

    return conversion_dict[instance_unit]


def convert_storage_to_instance_units(instance, category_name, value_name, value):
    """
    Convert given value from storage units to instance units.
    """
    if isinstance(value, Number) and is_convertible(category_name, value_name):
...
def get_storage_value(instance, category_name, value_name, value):
    if not isinstance(value, Number):
        return value
    return value / storage_to_instance_units_factor(instance, category_name, value_name)
