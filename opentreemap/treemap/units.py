    ...
    def units(self):
        ...
        for field in self._meta.get_fields():
            if self.instance and is_convertible(model, field.name):
                unit_abbreviation = get_unit_abbreviation(get_units(self.instance, model, field.name))
                unit_dict['{}.{}'.format(model, field.name)] = unit_abbreviation
