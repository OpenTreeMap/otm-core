...
class InstanceDataCommand(BaseCommand):
    ...
    def setup_env(self, *args, **options):
        ...
        if options["instance"]:
            ...
            instance = Instance.objects.get(url_name=options["instance_url_name"])
        else:
        ...
        if instance_user is None:
            ...
            instance_user = InstanceUser(instance=instance, user=user, role=r[0])
            instance_user.save_with_user(user)
            self.stdout.write("Added system user to instance with ADMINISTRATOR role")
        ...
        if options.get("delete", False):
            ...
            n_favorites = Favorite.objects.filter(
                map_feature__instance=instance
            ).count()
            with connection.cursor() as cursor:
                cursor.execute(
                    """
            ...
            n_comments = EnhancedThreadedComment.objects.filter(
                instance=instance
            ).count()
            with connection.cursor() as cursor:
                cursor.execute(
            ...
            n_rows = TreeImportRow.objects.filter(plot__instance=instance).count()
            with connection.cursor() as cursor:
                cursor.execute(
                    """
