...
def convert_units_all(to_imperial, apps, schema_editor):
    ...
    RainGarden.objects.all().update(
        drainage_area=F("drainage_area") * conversion_factor
    )
    Bioswale.objects.all().update(drainage_area=F("drainage_area") * conversion_factor)
