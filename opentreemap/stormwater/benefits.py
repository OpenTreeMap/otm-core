...
class PolygonalBasinBenefitCalculator(BenefitCalculator):
    ...
    def _benefits_for_feature_qs(self, feature_qs, instance):
        ...
        should_compute = (
            instance.annual_rainfall_inches is not None
            and diversion_rate is not None
            and config["should_show_eco"]
        )
        if should_compute:
            total_drainage_area = feature_qs.aggregate(
                total_drainage_area=Sum("drainage_area")
            ).get("total_drainage_area")
        ...
        if should_compute:
            ...
            feature_areas = self.MapFeatureClass.feature_qs_areas(poly_qs)
            ...
            basis = self._get_basis(features_used, feature_count - features_used)
        else:
            stats = {}
    ...
    def _get_basis(self, n_calc, n_discard):
        return {
            "resource": {"n_objects_used": n_calc, "n_objects_discarded": n_discard}
        }
