...
class PolygonalMapFeature(MapFeature):
    ...
    def polygon_area(cls, polygon):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT ST_Area(ST_Transform(ST_GeomFromEWKB(%s), 4326)::geography)",  # NOQA
                [polygon.ewkb],
            )
    ...
    def feature_qs_areas(cls, polygonal_map_feature_qs):
        ...
        area_col_name = "area"
        feature_areas = polygonal_map_feature_qs.extra(
            select={area_col_name: area_sql}
...
class Bioswale(PolygonalMapFeature, ValidationMixin):
    ...
    def clean(self):
        self.validate_positive_nullable_float_field("drainage_area", zero_ok=True)
