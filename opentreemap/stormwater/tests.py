...
from treemap.ecobenefits import (
    FEET_SQ_PER_METER_SQ,
    FEET_PER_INCH,
...
class ResourcePermsTest(PermissionsTestCase):
    ...
    def _create_rainbarrel_return_map_feature(self):
        ...
        rainbarrel = RainBarrel(instance=self.instance, geom=self.p, capacity=50.0)
    ...
    def test_model_is_creatable(self):
        self._add_builtin_permission(self.role_yes, RainBarrel, "add_rainbarrel")
        self.assertTrue(perms.model_is_creatable(self.role_yes, RainBarrel))

    def test_any_resource_is_creatable(self):
        self._add_builtin_permission(self.role_yes, RainBarrel, "add_rainbarrel")
        self.assertTrue(perms.any_resource_is_creatable(self.role_yes))

    def test_map_feature_is_not_creatable(self):
        self.assertFalse(perms.model_is_creatable(self.role_no, RainBarrel))

    def test_no_resource_is_creatable(self):
        self.assertFalse(perms.any_resource_is_creatable(self.role_no))

    def test_rainbarrel_photo_is_addable(self):
        self._add_builtin_permission(
            self.role_yes, MapFeaturePhoto, "add_rainbarrelphoto"
        )
    ...
    def test_rainbarrel_photo_is_not_addable(self):
        self._add_builtin_permission(self.role_no, RainBarrel, "add_rainbarrel")
        self._add_builtin_permission(self.role_no, Bioswale, "add_bioswale")
        self._add_builtin_permission(self.role_no, MapFeaturePhoto, "add_bioswalephoto")
        rainbarrel = self._create_rainbarrel_return_map_feature()
    ...
    def test_user_can_create_rainbarrel_photo(self):
        ...
        rainbarrel = self._create_rainbarrel_return_map_feature()
        user_yes = make_user(
            instance=self.instance, make_role=lambda inst: self.role_yes
        )
    ...
    def test_user_cannot_create_rainbarrel_photo(self):
        ...
        self._add_builtin_permission(self.role_no, MapFeaturePhoto, "add_bioswalephoto")
        rainbarrel = self._create_rainbarrel_return_map_feature()
        user_no = make_user(instance=self.instance, make_role=lambda inst: self.role_no)
        photo = MapFeaturePhoto(instance=self.instance, map_feature=rainbarrel)
        photo.set_image(self.load_resource("tree1.gif"))
    ...
    def test_rainbarrel_photo_is_deletable(self):
        ...
        self._add_builtin_permission(
            self.role_yes, MapFeaturePhoto, "delete_rainbarrelphoto"
        )
        user_yes = make_user(
            instance=self.instance, make_role=lambda inst: self.role_yes
        )
        self.assertTrue(
            perms.is_deletable(user_yes.get_instance_user(self.instance), photo)
        )
    ...
    def test_rainbarrel_photo_is_not_deletable(self):
        ...
        self._add_builtin_permission(self.role_no, RainBarrel, "delete_rainbarrel")
        self._add_builtin_permission(self.role_no, Bioswale, "delete_bioswale")
        self._add_builtin_permission(
            self.role_no, MapFeaturePhoto, "delete_bioswalephoto"
        )
        user_no = make_user(instance=self.instance, make_role=lambda inst: self.role_no)
        self.assertFalse(
            perms.is_deletable(user_no.get_instance_user(self.instance), photo)
        )
...
class PolygonalMapFeatureTest(OTMTestCase):
    def setUp(self):
        ...
        Bioswale.set_config_property(self.instance, "diversion_rate", 0.5)
    ...
    def _make_square_polygon(self, x, y):
        ...
        polygon = MultiPolygon(
            Polygon(
                ((x, y), (x, y + d), (x + d, y + d), (x + d, y), (x, y)), srid=4326
            ),
            srid=4326,
        )
    ...
    def _make_map_feature(
        self, MapFeatureClass, drainage_area=None, geom=None, polygon=None
    ):
        feature = MapFeatureClass(
    ...
    def test_calculate_area(self):
        ...
        self.assertAlmostEqual(
            bioswale.calculate_area(),
            self.polygon_area_sq_meters,
            delta=ASSERT_ALMOST_EQUAL_AREA_DELTA * self.polygon_area_sq_meters,
        )  # NOQA
    ...
    def _assert_runoff_reduced(
        self, area_sq_meters, drainage_area_sq_meters, diversion_rate, runoff_reduced
    ):
        ...
        self.assertAlmostEqual(
            runoff_reduced,
            expected,
            delta=ASSERT_ALMOST_EQUAL_RUNOFF_REDUCTION_DELTA * expected,
        )  # NOQA
    ...
    def test_rain_garden(self):
        ...
        self._assert_runoff_reduced(
            self.polygon_area_sq_meters, drainage_area_sq_meters, 0.85, runoff_reduced
        )
    ...
    def test_bioswale(self):
        ...
        self._assert_runoff_reduced(
            self.polygon_area_sq_meters, drainage_area_sq_meters, 0.5, runoff_reduced
        )
    ...
    def test_eco_not_wanted(self):
        ...
        drainage_area_sq_meters = 100000000.0
        feature = self._make_map_feature(
            RainGarden, drainage_area=drainage_area_sq_meters
        )
        runoff_reduced = self._get_runoff_reduced(feature, expect_empty=True)
        self.assertEqual(runoff_reduced, 0)
