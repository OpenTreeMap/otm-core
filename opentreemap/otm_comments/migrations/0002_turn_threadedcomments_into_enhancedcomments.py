...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for comment in orm['threadedcomments.ThreadedComment'].objects.values():
            ec = orm.EnhancedThreadedComment(**comment)

            # Not bothering to go through the content type system
            # Comments have only ever been enabled on MapFeatures
            mf = orm['treemap.MapFeature'].objects.get(pk=ec.object_pk)
            ec.instance_id = mf.instance_id

            ec.save()

        # EnhancedThreadedComment is "bumpy", so we shouldn't delete the old comments

    def backwards(self, orm):
        # Each EnhancedThreadedComment has a ThreadedComment, so don't do anything
        pass
    ...
    complete_apps = ['treemap', 'otm_comments']
    symmetrical = True
