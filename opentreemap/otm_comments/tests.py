...
from otm_comments.models import EnhancedThreadedComment, EnhancedThreadedCommentFlag
from otm_comments.views import (
    comment_moderation,
    flag,
    unflag,
    archive,
...
class CommentReviewTest(CommentTestCase):
    ...
    def test_archived_filter(self):
        ecomment1 = make_comment(
            self.plot, self.user, is_archived=True, is_removed=False
        )
        ecomment2 = make_comment(
            self.plot, self.user, is_archived=False, is_removed=True
        )
    ...
    def test_hidden_filter(self):
        ...
        ecomment3 = make_comment(
            self.plot, self.user, is_archived=False, is_removed=False
        )
...
class CommentModerationTestCase(CommentTestCase):
    def hit_flag_endpoint(self, user, params=None):
        req = make_request(params, user=user, instance=self.instance, method="POST")
...
class CommentFlagTest(CommentModerationTestCase):
    def test_flagging(self):
        ...
        updated_comment = EnhancedThreadedComment.objects.get(pk=self.comment.id)
        self.assertEqual(
            1,
    ...
    def test_flag_hide_flags_flag_makes_two_rows(self):
        ...
        req = make_request(
            _comment_ids_to_params(self.comment.id), user=self.admin, method="POST"
        )
...
class CommentArchiveTest(CommentModerationTestCase):
    def test_archive(self):
        ...
        req = make_request(
            _comment_ids_to_params(self.comment.id), user=self.user, method="POST"
        )
    ...
    def test_flagging_unarchives(self):
        ...
        self.hit_flag_endpoint(
            user=self.user, params=_comment_ids_to_params(self.comment.id)
        )
        updated_comment = EnhancedThreadedComment.objects.get(pk=self.comment.id)
        self.assertFalse(updated_comment.is_archived)
