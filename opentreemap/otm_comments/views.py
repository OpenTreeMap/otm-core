...
def get_comments(params, instance):
    ...
    comments = (
        EnhancedThreadedComment.objects.filter(content_type__model__in=types)
...
def flag(request, instance, comment_id):
    comment = EnhancedThreadedComment.objects.get(pk=comment_id, instance=instance)
    user_already_has_visible_flag = (
        len(
...
def hide_flags(request, instance):
    ...
    EnhancedThreadedCommentFlag.objects.filter(
        comment__id__in=comment_ids, comment__instance=instance
    ).update(hidden=True)
