...
class CommentReviewUITest(CommentTestMixin, TreemapUITestCase):
    def setUp(self):
        ...
        self.removed_comment = make_comment(self.plot, self.user, is_removed=True)
        ...
        self.comments_url = reverse(
            "comment_moderation_admin", args=(self.instance.url_name,)
        )
        self.browse_to_url(self.comments_url)

    def assert_num_rows(self, num, msg=None):
        rows = self.driver.find_elements_by_css_selector(".comment-table tbody tr")
    ...
    def go_to_page(self, page_num):
        ...
        self.driver.execute_script("return arguments[0].scrollIntoView();", page_link)
        page_link.click()
    ...
    def wait_until_on_page(self, page_num):
        ...
        page_num = page_num + 2
        self.wait_until_present(".pagination li:nth-child(%s).active" % page_num)
    ...
    def test_filtering(self):
        self.assert_num_rows(5, "There are 5 comments on this page before filtering")

        self.click('.page-header [data-toggle="dropdown"]')

        dropdown = self.driver.find_element_by_css_selector("[data-comments-filter]")
        ...
        id_link = self.find(".comment-table tbody").find_element_by_link_text(
            str(self.removed_comment.pk)
        )

        url = id_link.get_attribute("href")
        reversed_url = reverse(
            "map_feature_detail", args=(self.instance.url_name, self.plot.pk)
        )

        self.assertTrue(
            url.endswith(reversed_url), "The comment should link to the detail page"
        )
    ...
    def test_less_or_more(self):
        ...
        less_or_more_link = first_comment.find_element_by_css_selector(
            "[data-less-more]"
        )
    ...
    def test_archiving_single(self):
        ...
        self.find(".comment-table tbody tr").find_element_by_link_text(
            "Archive"
        ).click()
        ...
        self.assertEqual(
            1, EnhancedThreadedComment.objects.filter(is_archived=True).count()
        )

    def test_archiving_batch(self):
        checkboxes = self.driver.find_elements_by_css_selector(
            "[data-batch-action-checkbox]"
        )
        ...
        self.find("[data-batch-action]").find_element_by_link_text("Archive").click()

        sleep(3)

        self.assertEqual(
            5, EnhancedThreadedComment.objects.filter(is_archived=True).count()
        )
...
class CommentUITest(CommentTestMixin, TreemapUITestCase):
    ...
    def assertCommentText(self, pk, comment_text):
        comment_text_el = self.find_id("c" + str(pk)).find_element_by_css_selector(
            ".comment_text"
        )
        self.assertEqual(comment_text_el.text, comment_text)

    def click_post_button(self):
        (self.find(".comment-create-form").find_element_by_name("post").click())
