...
REQUIRED_SETTINGS = ("IE_VERSION_MINIMUM", "IE_VERSION_UNSUPPORTED_REDIRECT_PATH")
REQUIRED_SETTING_MSG = (
    "InternetExplorerRedirectMiddleware is loaded " "but %s was not found in settings."
)
...
class InternetExplorerRedirectMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        required_setting_msgs = [
            REQUIRED_SETTING_MSG % setting
            for setting in REQUIRED_SETTINGS
            if not hasattr(settings, setting)
        ]

        required_parameter_msgs = [
            REQUIRED_PARAMETER_MSG % parameter
