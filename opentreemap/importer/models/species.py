...
class SpeciesImportEvent(GenericImportEvent):
    ...
    def instance_region_codes(self):
        if getattr(self, "_instance_region_codes", None) is None:
            self._instance_region_codes = [
                itr.code for itr in self.instance.itree_regions()
            ]
    ...
    def legal_and_required_fields(self):
        return (fields.species.ALL, {fields.species.GENUS, fields.species.COMMON_NAME})
...
class SpeciesImportRow(GenericImportRow):
    ...
    def diff_from_species(self, species):
        ...
        if key in data:
            ...
            model_itree_pairs = [
                (region, species.get_itree_code(region))
    ...
    def validate_itree_code_field(self):
        ...
        if ":" in itree_code:
            ...
            # SoCalCSMA:CEL OTHER, InlEmpCLM:CEL OTHER
            codes = [pair.split(":") for pair in itree_code.split(",")]
        ...
        if error:
            self.append_error(
                ...
                {"region": pairs[-1][0], "code": pairs[-1][1]},
            )
        else:
        ...
        if len(possible_matches) == 0:
            ...
            if all(diff.keys() == ["id"] for diff in diffs):
                ...
                if len(diff_keys) == 0:
                    ...
                    self.append_error(
                        errors.MERGE_REQUIRED, tuple(diff_keys), filtered_diffs
                    )
    ...
    def commit_row(self):
        ...
        self.convert_units(
            data,
            {
                fields.species.MAX_DIAMETER: self.import_event.max_diameter_conversion_factor,
                fields.species.MAX_HEIGHT: self.import_event.max_tree_height_conversion_factor,
            },
        )
