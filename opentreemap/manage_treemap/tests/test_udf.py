...
from manage_treemap.views.udf import udf_list, udf_create, udf_delete, udf_bulk_update
...
def _do_create_udf_request(body, instance):
    return udf_create(make_request(method="POST", body=json.dumps(body)), instance)
...
class UdfReadUpdateTest(UdfCRUTestCase):
    def test_get_udfs(self):
        ...
        plain_udfs = [
            s["udf"] for s in plot_udfs[0]["specs"] if not s["udf"].iscollection
        ]
...
class UdfBulkUpdateTest(OTMTestCase):
    def setUp(self):
        ...
        self.plot_stew, self.tree_stew = tuple(create_stewardship_udfs(self.instance))

        self.user = make_commander_user(self.instance)

    def _make_put_request(self, params):
        request = RequestFactory().put(
            "does/not/matter/", json.dumps(params), content_type="application/json"
        )
        request.method = "PUT"
    ...
    def _make_point(self, fraction):
        ...
        return Point(
            (maxx - self.instance.center.x) * fraction + self.instance.center.x,
            (maxy - self.instance.center.y) * fraction + self.instance.center.y,
    ...
    def test_bulk_update_scalar_choices(self):
        plot_udfd = UserDefinedFieldDefinition.objects.create(
            ...
            model_type="Plot",
            datatype=json.dumps({"type": "choice", "choices": ["a", "b"]}),
            iscollection=False,
        ...
        tree_udfd = UserDefinedFieldDefinition.objects.create(
            instance=self.instance,
            model_type="Tree",
            datatype=json.dumps({"type": "choice", "choices": ["x", "y"]}),
            iscollection=False,
            name="Test tree choice",
        )

        set_write_permissions(
            self.instance, self.user, "Plot", ["udf:Test plot choice"]
        )

        set_write_permissions(
            self.instance, self.user, "Tree", ["udf:Test tree choice"]
        )
...
class UdfDeleteTest(OTMTestCase):
    def setUp(self):
        ...
        set_write_permissions(
            self.instance, self.user, "Plot", ["udf:Test choice", "udf:Stewardship"]
        )
    ...
    def test_delete_udf_deletes_perms(self):
        ...
        qs = FieldPermission.objects.filter(
            field_name="udf:cool udf", model_name="Plot"
        )
    ...
    def test_delete_scalar_udf(self):
        ...
        self.assertFalse(
            UserDefinedFieldDefinition.objects.filter(pk=self.udf.pk).exists()
        )
    ...
    def test_cant_delete_collection_udf(self):
        ...
        self.assertTrue(
            UserDefinedFieldDefinition.objects.filter(pk=self.cudf.pk).exists()
        )
