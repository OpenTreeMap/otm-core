...
class AddMapFeatureTypeTest(OTMTestCase):
    ...
    def test_exception_adding_duplicate(self):
        self.assertRaises(
            ValidationError, self.instance.add_map_feature_types, ["Plot"]
        )

    def test_exception_adding_nonexistent(self):
        self.assertRaises(
            ValidationError, self.instance.add_map_feature_types, ["IDoNotExist"]
        )
    ...
    def test_type_and_permissions_are_added(self):
        ...
        self.assertEqual(
            set(instance.map_feature_types), current_map_feature_types | {"RainGarden"}
        )
...
class GreenInfrastructureUpdateTest(OTMTestCase):
    ...
    def test_add_map_feature_types(self):
        ...
        request = make_request(method="PUT", body=json_updates, user=self.commander)
    ...
    def test_map_feature_applicable_constant_validation(self):
        ...
        request_key = "instance.config.map_feature_config.Bioswale" ".diversion_rate"
    ...
    def test_map_feature_inapplicable_config_validation(self):
        ...
        request_key = "instance.config.map_feature_config.RainBarrel" ".should_show_eco"
        updates = {request_key: True}
        json_updates = json.dumps(updates)
        request = make_request(method="PUT", body=json_updates, user=self.commander)
    ...
    def test_show_eco_without_rainfall(self):
        ...
        Bioswale.set_config_property(self.instance, "diversion_rate", 0.9)

        request_key = "instance.config.map_feature_config.Bioswale" ".should_show_eco"
        updates = {request_key: True}
        json_updates = json.dumps(updates)
        request = make_request(method="PUT", body=json_updates, user=self.commander)
