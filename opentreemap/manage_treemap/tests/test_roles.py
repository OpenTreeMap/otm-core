...
class UserRolesTest(OTMTestCase):
    def setUp(self):
        ...
        self.user4 = User(username="faxe", password="faxe", email="faxe@example.com")
    ...
    def _add_user_to_instance_view(self, email):
        ...
        return create_user_role(
            make_request(method="POST", body=json.dumps(body), user=self.commander),
            self.instance,
        )
    ...
    def test_email_already_bound(self):
        iuser = InstanceUser(
            user=self.user1, instance=self.instance, role=self.instance.default_role
        )
    ...
    def test_invites_updated(self):
        ...
        new_role = Role(name="Ambassador", instance=self.instance, rep_thresh=0)
        new_role.save()
    ...
    def test_user_roles_updated(self):
        iuser = InstanceUser(
            user=self.user2, instance=self.instance, role=self.instance.default_role
        )
        iuser.save_with_user(self.commander)
...
class ModelPermMgmtTest(OTMTestCase):
    ...
    def test_unassignment(self):
        ...
        self.assertEqual(
            self.RolePermissionModel.objects.filter(role=self.new_role).count(), 2
        )

        self.request_updates(
            {"Instance.do_all_the_things": False, "Plot.add_plot": False}
        )
...
class FieldPermMgmtTest(OTMTestCase):
    ...
    def assertUpdatesRaisesValidation(self, updates):
        json_updates = json.dumps(updates)
        request = make_request(method="PUT", body=json_updates)
        self.assertRaises(ValidationError, roles_update, request, self.instance)
    ...
    def test_invalid_model_not_authorizable_integration(self):
        updates = self.make_updates(self.new_role.pk, {"FieldPermission.model_name": 2})
        self.assertUpdatesRaisesValidation(updates)
