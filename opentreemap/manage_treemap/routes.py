...
from manage_treemap.views.udf import (
    udf_bulk_update,
    udf_create,
    udf_list,
    udf_delete_popup,
...
site_config = admin_route(
    GET=do(render_template("manage_treemap/basic.html"), views.site_config_basic_info),
    PUT=json_do(update_instance_fields_with_validator, views.site_config_validator),
)

external_link = admin_route(
    GET=do(render_template("manage_treemap/link.html"), views.external_link),
    PUT=json_do(views.update_external_link),
)

branding = admin_route(
    GET=do(render_template("manage_treemap/branding.html"), views.branding),
    PUT=json_do(update_instance_fields_with_validator, views.branding_validator),
)

embed = admin_route(GET=do(render_template("manage_treemap/embed.html"), views.embed))
...
green_infrastructure = admin_route(
    GET=do(
        render_template("manage_treemap/green_infrastructure.html"),
...
comment_moderation = admin_route(
    GET=do(
        render_template("manage_treemap/comment_moderation.html"),
...
photo_review_admin = admin_route(
    GET=do(
        render_template("manage_treemap/photo_review.html"), photo_views.photo_review
    )
)
...
user_roles = admin_route(
    GET=do(render_template("manage_treemap/user_roles.html"), user_roles_list),
...
user_roles_partial = admin_route(
    GET=do(render_template("manage_treemap/partials/user_roles.html"), user_roles_list)
)

user_invites = admin_route(DELETE=remove_invited_user_from_instance)

begin_export_users = do(json_api_call, admin_instance_request, begin_export_users)

roles = admin_route(
    GET=do(render_template("manage_treemap/roles.html"), roles_list),
    PUT=do(roles_update),
    POST=do(render_template("manage_treemap/partials/roles.html"), roles_create),
)

clear_udf_notifications = admin_route(POST=do(json_api_call, remove_udf_notifications))

importer = admin_route(
    GET=do(render_template("manage_treemap/importer.html"), list_imports)
)

benefits = admin_route(
    GET=do(render_template("manage_treemap/benefits.html"), views.benefits_convs),
    PUT=json_do(views.update_benefits),
)

units = admin_route(
    GET=do(render_template("manage_treemap/units.html"), views.units),
    PUT=json_do(update_instance_fields_with_validator, views.units_validator),
)
...
udfs = do(
    ...
    route(
        ...
        GET=do(render_template("manage_treemap/udfs.html"), udf_list),
    ),
)
...
udf_change = do(
    ...
    route(
        GET=do(
            render_template("manage_treemap/partials/fields/udf_delete_popup.html"),
            udf_delete_popup,
        ),
...
search_config_page = admin_route(
    GET=do(
        render_template("manage_treemap/search_fields.html"), field_views.search_config
    )
)
...
search_config = do(
    ...
    route(
        ...
        GET=do(
            render_template("manage_treemap/partials/fields/search_fields.html"),
...
field_configs = admin_route(
    GET=do(
        render_template("manage_treemap/field_configuration.html"),
...
set_field_configs = do(
    admin_instance_request,
    route(
        PUT=do(json_api_call, return_400_if_validation_errors, field_views.set_fields),
        GET=do(
            render_template("manage_treemap/partials/fields/field_groups.html"),
            field_views.set_fields_page,
        ),
    ),
)
