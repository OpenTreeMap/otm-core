...
from treemap.plugin import (
    can_add_user,
    does_user_own_instance,
...
def user_roles_list(request, instance):
    ...
    invited = (
        instance.instanceinvitation_set.select_related("role")
        .filter(accepted=False)
        .order_by(invite_sort, "email")
    )
    instance_users = (
        instance.instanceuser_set.select_related("role", "user")
...
def invite_user_with_email_to_instance(request, email, instance):
    ...
    if existing_invites.exists():
        raise ValidationError(
            _("A user with email address '%s' has already been invited.") % email
        )
...
def remove_invited_user_from_instance(request, instance, invite_id):
    invite = get_object_or_404(InstanceInvitation, pk=invite_id, accepted=False)
    invite.delete()
...
def add_user_to_instance(request, user, instance, admin_user):
    iuser_already_exists = InstanceUser.objects.filter(
        user_id=user.pk, instance=instance
    ).exists()

    if iuser_already_exists:
        raise ValidationError(
            _("A user with email address '%s' has already " "joined this map.")
            % user.email
        )

    iuser = InstanceUser(user_id=user.pk, instance=instance, role=instance.default_role)
...
def update_user_roles(request, instance):
    ...
    for Model, key in ((InstanceUser, "users"), (InstanceInvitation, "invites")):
        ...
        for pk, updated_info in updates.iteritems():
            ...
            if model.role_id != updated_role or is_admin != model.admin:
                ...
                if Model == InstanceInvitation:
                    model.save()
                elif does_user_own_instance(instance, model.user) and not is_admin:
...
def should_send_user_activation(request, username, email, password):
    ...
    return not InstanceInvitation.objects.filter(
        email__iexact=email, activation_key=activation_key
    ).exists()
...
def activate_user(sender, user, request, password=None, **kwargs):
    ...
    invites_matching_email = InstanceInvitation.objects.filter(email__iexact=user.email)
    ...
    invites_matching_key = InstanceInvitation.objects.filter(activation_key=key)
    ...
    if invites_matching_key:
        ...
        if this_invite.email.lower() == user.email.lower():
            ...
            auser = authenticate(username=user.username, password=password)
            login(request, auser)
        else:
...
def create_instance_users(sender, user, request, *args, **kwargs):
    ...
    invites = InstanceInvitation.objects.filter(
...
def create_instance_users_from_invites(request, user, invites):
    ...
    for invite in invites:
        ...
        iuser = InstanceUser(
            user=user, instance=instance, role=invite.role, admin=invite.admin
        )
        ...
        emails_to_notify = set(
            [invite.created_by.email] + invitation_accepted_notification_emails(invite)
        )
