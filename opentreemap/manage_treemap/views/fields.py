...
from treemap.search_fields import (
    set_search_field_label,
    ALERT_IDENTIFIER_PATTERN,
    get_alert_field_info,
)
...
def set_fields_page(request, instance):
    ...
    def get_disabled_fields(group):
        ...
        model_fields = {
            field for field in mobj.tracked_fields if _should_show_field(Model, field)
        }
    ...
    def get_disabled_cudfs(group):
        ...
        udfs = {
            ...
            if udf.iscollection
            and udf.model_type in models
            and (
                group["header"] not in collection_groups or udf.name == group["header"]
            )
        }
...
def _website_search_config(instance):
    ...
    fields_by_model = {
        model_name: sorted(_get_fields(instance, model_name))
        for model_name in model_names
    }

    def get_context_for_model(model_name):
        model_fields = fields_by_model[model_name]
        enabled, disabled = get_enabled_and_disabled_fields(model_name, model_fields)
    ...
    def get_context_for_missing():
        ...
        enabled, disabled = get_enabled_and_disabled_fields("missing", all_fields)
    ...
    field_groups = [get_context_for_model(model_name) for model_name in model_names]
...
def _mobile_search_config(instance):
    ...
    def get_enabled_and_disabled_fields(category, fields):
        ...
        return (
            _add_field_info(instance, enabled_fields),
    ...
    def alert_identifiers():
        def identifier(udf):
            ...
            return "udf:%(model)s:%(pk)s.Status" % {"model": model_name, "pk": udf.pk}
    ...
    def add_alert_info(field_identifiers):
        return [
            get_alert_field_info(id) if ALERT_IDENTIFIER_PATTERN.match(id) else id
...
def _get_fields(instance, model_name):
    ...
    model_fields = {"%s.%s" % (to_object_name(model_name), f) for f in model_fields}
    return model_fields
