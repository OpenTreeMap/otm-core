...
PEND_PERM = (_("Pending Write Access"), FieldPermission.WRITE_WITH_AUDIT)
READONLY_PERM = (_("Read Only"), FieldPermission.READ_ONLY)
INVISIBLE_PERM = (_("Invisible"), FieldPermission.NONE)
...
def field_perm_models(instance):
    return {Tree} | {MapFeature.get_subclass(m) for m in instance.map_feature_types}
...
def _update_perms_from_object(role_perms, instance):
    ...
    valid_perm_models_by_name = {m.__name__: m for m in model_perm_models(instance)}
    roles_by_id = {role.pk: role for role in Role.objects.filter(instance=instance)}
    field_perms = {
        (role, perm.full_name): perm
    ...
    input_role_fields = zip(
        input_roles, [role_inputs["fields"] for role_inputs in role_perms.itervalues()]
    )
    input_role_models = zip(
        input_roles, [role_inputs["models"] for role_inputs in role_perms.itervalues()]
    )
        ...
        for model_field_name, perm_type in field_perm.iteritems():
            ...
            if create or field_perm.permission_level != perm_type:
                ...
                if perm_type in valid_levels:
                    ...
                    raise Exception(
                        "Invalid field type "
                        "(allowed %s, given %s)" % (valid_levels, perm_type)
                    )
        ...
        try:
            ...
            raise ValidationError(
                "{} is not a valid codename for {}.{}".format(
                    codename, app_config.models_module.__name__, app_config.label
                )
            )
    ...
    def validate_and_save_model_perm(role, model_perm):
        ...
        for model_perm_name, should_be_assigned in model_perm.iteritems():
            ...
            validate_model_name(model_name, set(valid_perm_models_by_name.keys()))
        ...
        if unassign:
            unassign_q = reduce(lambda q1, q2: q1 | q2, [Q(**rpm) for rpm in unassign])
...
def roles_list(request, instance):
    ...
    model_permissions = {
        Model: Permission.objects.filter(
            Q(codename__startswith="add") | Q(codename__startswith="delete"),
    ...
    photo_class_for_model = (
        lambda Model: TreePhoto if Model is Tree else MapFeaturePhoto
    )
    ...
    def get_field_perms(role, Model):
        ...
        model_perms = [
            fp
            for fp in role_field_permissions(role, instance, model_name)
    ...
    def get_role_model_perms(role, Model):
        return _get_role_model_perms(model_permissions[Model], role, Model)
    ...
    def get_instance_perms(roles):
        def translated(spec):
            return {
                k: unicode(v) if isinstance(v, Promise) else v
        ...
        role_instance_perms = [
            combine(
                {"role": role},
                {perm.codename: perm for perm in role.instance_permissions.all()},
            )
            for role in roles
        ]

        specs = sorted(
            [translated(spec) for spec in get_instance_permission_spec(instance)],
...
def roles_create(request, instance):
    ...
    if not role_name:
        return HttpResponseBadRequest(_("Must provide a name for the new role."))

    role, created = Role.objects.get_or_create(
        name=role_name, instance=instance, rep_thresh=0
    )
