...
def get_photos(instance, sort_order="-created_at", is_archived=False):
    unverified_actions = {Audit.Type.Insert, Audit.Type.Delete, Audit.Type.Update}

    audit_model_ids = Audit.objects.filter(
        instance=instance,
...
def approve_or_reject_photos(request, instance, action):
    ...
    for photo_id in photo_ids:
        try:
            photo = MapFeaturePhoto.objects.select_related("treephoto").get(pk=photo_id)
            try:
