...
def _get_replaceable_models(instance):
    ...
    root_classes = {Cls for Cls in all_classes if not Cls.__name__.endswith("LA")}
    ...
    if instance.url_name == "latreemap":
        classes = la_classes | {
            ...
            if not {
                LACls for LACls in la_classes if LACls.__name__.startswith(Cls.__name__)
            }
        }
    else:
...
def site_config_green_infrastructure(request, instance):
    ...
    __, annual_rainfall_display_value = get_display_value(
        instance, "greenInfrastructure", "rainfall", instance.annual_rainfall_inches
    )
    ...
    for Cls in customizable_models:
        ...
        (
            gsi_model["diversion_rate"],
            gsi_model["diversion_rate_applies"],
        ) = _get_diversion_rate_for_display(config)
        (
            gsi_model["should_show_eco"],
            gsi_model["should_show_eco_applies"],
...
def _map_feature_config_validator(field_name, value, instance):
    acceptable_terms = [Cls.__name__ for Cls in _get_replaceable_models(instance)]

    __, __, term, key = dotted_split(field_name, 4, maxsplit=3)
    field_name_valid = term in acceptable_terms and key in (
        "should_show_eco",
        "diversion_rate",
    )
    ...
    plural = get_attr_from_json_field(instance, "config.terms.{}.plural".format(term))

    if key == "should_show_eco":
        if term == "RainBarrel":
            return [
                _("Showing Ecobenefits is not applicable to {RainBarrels}").format(
                    RainBarrels=plural
                )
            ]
        ...
        if term == "RainBarrel":
            return [
                _("The runoff coefficient is not applicable to {RainBarrels}").format(
                    RainBarrels=plural
                )
            ]
        error_message = (
            "Please enter a number between 0 and 1 " "for the runoff coefficient"
        )
    ...
    if setting == "should_show_eco":
        ...
        if value:
            ...
            if rainfall_inches is None or rainfall_inches == "":
                return [
                    _(
                        "Ecobenefits cannot be calculated unless "
...
def _terminology_validator(field_name, value, instance):
    ...
    field_name_valid = (
        terms == "terms" and term in acceptable_terms and form in ("singular", "plural")
    )
    if not field_name_valid:
        return [_("An invalid key was sent in the request")]
...
def _annual_rainfall_validator(value):
    INVALID_RAINFALL_MESSAGE = _("Please enter a positive number for annual rainfall")
    if value is None or value == "":
...
def _set_annual_rainfall(value, instance):
    value = get_storage_value(instance, "greenInfrastructure", "rainfall", float(value))
...
def green_infrastructure(request, instance):
    ...
    for identifier, value in json_data.iteritems():
        ...
        if field_name.startswith("config.map_feature_types") or field_name.startswith(
            "config.map_feature_config"
        ):
            if not instance.feature_enabled("green_infrastructure"):
                raise PermissionDenied(
                    "The Green Infrastructure module is "
