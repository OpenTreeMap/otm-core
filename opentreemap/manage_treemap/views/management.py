...
from treemap.lib.external_link import (
    get_url_tokens_for_display,
    validate_token_template,
)
from treemap.models import BenefitCurrencyConversion, Plot, Tree
from treemap.units import get_value_display_attr, get_convertible_units, get_unit_name
...
def site_config_basic_info(request, instance):
    info = {
        "url": request.build_absolute_uri(settings.SITE_ROOT + instance.url_name),
        "fields": [
    ...
    info["fields"].append((_("Contact E-Mail"), "instance.config.linkData.contact"))

    info["fields"].append((_("Tree ID URL"), "instance.config.linkData.treekey"))
    ...
    return {"instance": instance, "site_config": info}
...
def external_link(request, instance):
    return {"instance": instance, "tokens": get_url_tokens_for_display(True)}
...
def update_external_link(request, instance):
    def validator(field_name, value, model_name):
        ...
        if field_name not in {"config.externalLink.text", "config.externalLink.url"}:
            return [_("Invalid field name")]
            ...
            if not validate_token_template(value):
                return [
                    _(
                        "Invalid token, the allowed values are"
    ...
    url = data.get(
        "instance.config.externalLink.url", instance.config.get("externalLink.url")
    )
    text = data.get(
        "instance.config.externalLink.text", instance.config.get("externalLink.text")
    )
    if bool(url) ^ bool(text):
        ...
        raise ValidationError(
            {
                "instance.config.externalLink.%s"
                % field: [
...
def branding_validator(field_name, value, model_name):
    ...
    if field_name not in {
        "config.scss_variables.primary-color",
...
def embed(request, instance):
    ...
    iframe = '<iframe src="{}?embed=1" width="%s" height="%s"></iframe>'.format(
        embed_url
    )
    wide = {"w": "1280", "h": "888"}
...
def benefits_convs(request, instance):
    ...
    field_groups = {
        _("Energy Factors"): {
            ...
            _(
                "per kBTU of natural gas"
            ): "benefitCurrencyConversion.natural_gas_kbtu_to_currency",
        },
        _("Stormwater Factors"): {
            _(
                "per gallon of stormwater reduced"
            ): "benefitCurrencyConversion.h20_gal_to_currency",
        },
        _("CO₂ Factors"): {
            _("per lb of CO₂"): "benefitCurrencyConversion.co2_lb_to_currency",
        },
        _("Air Quality Factors"): {
            _("per lb of O₃"): "benefitCurrencyConversion.o3_lb_to_currency",
            _("per lb of NOₓ"): "benefitCurrencyConversion.nox_lb_to_currency",
            _("per lb of PM10"): "benefitCurrencyConversion.pm10_lb_to_currency",
            _("per lb of SOₓ"): "benefitCurrencyConversion.sox_lb_to_currency",
            _("per lb of VOC"): "benefitCurrencyConversion.voc_lb_to_currency",
        },
    }

    pfx = '<span class="currency-value">' + conv.currency_symbol + "</span> "

    for group_title, fields in field_groups.iteritems():
        fields_with_pfx = [
            ((pfx + label), value) for label, value in fields.iteritems()
        ]
        field_groups[group_title] = fields_with_pfx

    return {"benefitCurrencyConversion": conv, "benefit_fields": field_groups}
...
def update_benefits(request, instance):
    ...
    valid_fields = ["benefitCurrencyConversion." + field for field in valid_fields]
    ...
    for field, value in updated_values.iteritems():
        if field in valid_fields:
            ...
            raise Exception("invalid field specified %s for benefit conversion" % field)

    try:
        conv.save()
    except ValidationError as e:
        raise ValidationError(package_field_errors("benefitCurrencyConversion", e))
...
def units(request, instance):
    ...
    value_names = [
        ...
        {
            "title": _("{bioswale} Fields").format(
        ...
        {
            "title": _("{rainBarrel} Fields").format(
        ...
        {
            "title": _("{rainGarden} Fields").format(
    ...
    def get_label_getter(attrs):
        ...
        if Model:
            ...
            if name == "Tree" or name in instance.map_feature_types:
                return lambda value_name: Model._meta.get_field(value_name).verbose_name
        else:
    ...
    for attrs in value_names:
        ...
        for value_name in attrs["value_names"]:
            ...
            def identifier_and_value(key):
                return get_value_display_attr(instance, category_name, value_name, key)
            ...
            choices = [
                {"value": abbrev, "display_value": get_unit_name(abbrev)}
            ...
            category_specs.append(
                {"label": label, "units": units_field_spec, "digits": digits_field_spec}
            )

        if category_specs:
            specs_by_category.append({"title": attrs["title"], "items": category_specs})
